# specslice_dart_analyzer (P7 sidecar)

A Dart subprocess that uses `package:analyzer` to produce a resolved-AST
`LanguageIndexBatch` for the SpecSlice Rust engine. When this sidecar is
available, the engine prefers its output over the lightweight heuristic
adapter shipped at `crates/specslice-lang-dart/`.
...
Set `SPECSLICE_DART_ANALYZER=1` before running `specslice index`. The
engine looks for the sidecar at:

1. `$SPECSLICE_DART_ANALYZER_BIN` (a shell-style command, e.g.
   `dart run /path/to/bin/specslice_dart_analyzer.dart`).
2. Otherwise: `dart run tool/specslice_dart_analyzer/bin/specslice_dart_analyzer.dart`
...
```bash
cd tool/specslice_dart_analyzer
dart pub get
echo '{"repo_root":"/path/to/repo","code_roots":["lib"]}' \
  | dart run bin/specslice_dart_analyzer.dart \
  | jq '.references[0]'
```
