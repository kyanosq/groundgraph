...
void main() {
  test('walkRepository emits test and group artifacts', () async {
    final root = await Directory.systemTemp.createTemp('groundgraph_sidecar_');
...
''');
  ...
  test(
    ...
    // lint scope. GroundGraph has its own exclude config for graph scoping.
    final root = await Directory.systemTemp.createTemp('groundgraph_sidecar_');
    addTearDown(() => root.delete(recursive: true));
...
''');
  ...
  group('sanitizeDiagnosticText (#103)', () {
    test('collapses POSIX home dirs (username leak removed)', () {
      const raw =
          'PathNotFoundException: open failed, path = /Users/qjs/Code/groundgraph/tool/x.dart';
      final out = sanitizeDiagnosticText(raw);
      expect(out.contains('/Users/qjs/'), isFalse, reason: out);
      expect(out, contains('<home>/Code/groundgraph/tool/x.dart'));
    });
  ...
  test('walkRepository does not leak its options temp dir (#126)', () async {
    int optsDirCount() => Directory.systemTemp
        .listSync()
        .whereType<Directory>()
        .where((d) => p.basename(d.path).startsWith('groundgraph_opts_'))
        .length;
    ...
    expect(optsDirCount(), before,
        reason: 'walkRepository must reclaim its groundgraph_opts_* temp dir');
  });
  ...
  test('sidecar sanitises absolute paths out of serialised error detail (#103)',
    ...
    final fake =
        '/Users/groundgraph_fake_user_xyz/nope_${DateTime.now().microsecondsSinceEpoch}.json';
    final result = await Process.run(
      Platform.resolvedExecutable,
      ['run', 'bin/groundgraph_dart_analyzer.dart', '--request', fake],
    ...
    expect(detail.contains('groundgraph_fake_user_xyz'), isFalse,
  ...
  group('resolveSdkPath', () {
    ...
    test('recovers a valid SDK via PATH when the executable path is bogus', () {
      ...
      final recovered = resolveSdkPath(
        resolvedExecutable: '/tmp/groundgraph_bogus_sidecar_exe',
    ...
    test('prefers GROUNDGRAPH_DART_SDK override when the default is bogus', () {
      ...
      final picked = resolveSdkPath(
        resolvedExecutable: '/tmp/groundgraph_bogus_sidecar_exe',
        environment: {'GROUNDGRAPH_DART_SDK': realSdk, 'PATH': ''},
      );
      expect(picked, realSdk);
    });

    test('ignores old SPECSLICE_DART_SDK override', () {
      final realSdk = resolveSdkPath(
        resolvedExecutable: '/tmp/groundgraph_bogus_sidecar_exe',
        environment: Platform.environment,
      );
