...
/// "accuracy core" of the GroundGraph Dart adapter — when this is
/// available, the Rust engine prefers its output over the heuristic
/// fallback (`crates/groundgraph-lang-dart`).
///
/// Design constraints:
...
/// - AOT-compiled binary (`GROUNDGRAPH_DART_ANALYZER_BIN=/path/to/exe`) —
...
///   honour `GROUNDGRAPH_DART_SDK` / `DART_SDK`, or locate `dart` on `PATH` and
...
Future<SidecarBatchResponse> walkRepository(SidecarRequest req) async {
  ...
  // paths (e.g. `.` from `groundgraph --repo-root .`) throw inside the
  ...
  // roots, not the project's lint scope — GroundGraph has its own exclude config
  ...
  final optionsDir = await Directory.systemTemp.createTemp('groundgraph_opts_');
  ...
  // exit leaked one `groundgraph_opts_*` dir per sidecar invocation.
  try {
...
/// package:test, flutter_test, or local wrappers, and GroundGraph only needs a
