...
def main():
    ...
    print("> `./target/release/groundgraph --repo-root release-scans/_scratch/<repo>` 直跑生成。")
    ...
    print("`crates/groundgraph-engine/src/edge_confidence.rs:174` 的现实一致 ——")
    ...
    print()
    print("- `groundgraph-cli/src/commands/search.rs::parse_kind` 的 P20 补丁只补了")
    ...
    print()
    print("```bash")
    print("cargo build -p groundgraph-cli --release")
    print("for repo in pixcraft-app atagent pixcraft-landing vub; do")
    print("  ./target/release/groundgraph --repo-root release-scans/_scratch/$repo \\")
    print("    dead-code --json --min-confidence low \\")
    print("    > reports/release-v0.3.0a/$repo/dead-code-low.json")
    print("done")
    print("./target/release/groundgraph --repo-root release-scans/_scratch/pixcraft-app \\")
