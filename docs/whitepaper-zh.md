...
**它是什么**:一个命令行工具。把你的代码仓库(代码、文档、测试、SQL、路由)扫描成一张**证据图**,存进仓库里的一个 SQLite 文件(`.specslice/graph.db`),然后在这张图上回答各种问题。

**它能回答什么**:

| 你想知道 | 用哪个命令 |
| --- | --- |
| "下单逻辑在哪?"(中文也行) | `specslice search 下单` |
| "改了这两个文件,会影响哪些需求和测试?" | `specslice impact` |
| "这个接口往下调了谁?最后写哪张表?" | `specslice trace /api/orders` |
| "哪些代码没人调用,可以删?" | `specslice dead-code` |
| "这个仓库有哪些业务模块?" | `specslice propose` |
| "文档里写的代码路径还存在吗?" | `specslice check` |

**它不做什么**:不改你的任何一行代码、文档、测试(全部状态只写在 `.specslice/` 里,删掉即彻底清除);不上传任何数据(零网络);不内置任何 AI 模型(它产出"证据 + 提示词",由你自己的 AI 客户端消费)。
...
# 3. 开始提问
specslice search 支付回调        # 中文/英文都可以
specslice impact                 # 当前分支改动影响了什么
specslice dashboard              # 生成一个离线 HTML 总览面板
```
