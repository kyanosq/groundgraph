...
| 5 | **MCP 内嵌保鲜仍缺**:`groundgraph watch` 已可单独运行,但 MCP server 还不会自己 catch-up / 标记 stale | Agent 长会话中仍需用户另起 watcher 或手动 `index` | 下一步把 watcher 接入 MCP 启动/查询路径,补 staleness banner 与连接时 catch-up |
...
2. **P1**:MCP 内嵌保鲜、business 命令组合并、CI 模板;
