...
GroundGraph 的产品体验不是让用户手写业务逻辑与代码的映射，也不是用规则从文件名、标题、注释或命名约定里猜业务含义。产品闭环应当是：

1. **代码图生成事实。** GroundGraph 只用确定性索引器生成文档段落、Dart 符号、测试、调用、Provider、路由、持久化、Stream 等事实节点和事实边。
...
4. **确认后进入 confirmed graph。** 只有人工确认后的候选才会写入 `.groundgraph/requirements.yaml` 或 `.groundgraph/links.yaml`，并进入 confirmed graph。AI 候选默认不参与 `slice`、`context`、`impact` 的可信链路。
...
- 不能要求用户在业务代码、业务文档或测试中加入 GroundGraph 注解。
...
- GroundGraph 只能写 `.groundgraph.yaml`、`.groundgraph/links.yaml`、`.groundgraph/graph.db`、`.groundgraph/export/`，以及后续 `.groundgraph/requirements.yaml`、`.groundgraph/candidates/`。
...
- 人工负责确认、编辑、拒绝 AI 候选；确认结果写入 GroundGraph 自有目录。
...
- Rust workspace：`groundgraph-core`、`groundgraph-store`、`groundgraph-lang-dart`、`groundgraph-engine`、`groundgraph-cli`。
- `groundgraph init` 生成 `.groundgraph.yaml` 和 `.groundgraph/graph.db`。
- SQLite migration 创建 `nodes`、`edge_assertions`、`evidence`、`symbol_ranges`、`file_index`、`slice_cache`。
- `groundgraph export --format jsonl` 导出当前图数据。

**TDD 起点：**

- 先写 CLI e2e 测试：空目录执行 `groundgraph init` 后能看到配置和数据库。
...
- 先断言 `groundgraph index --docs-only` 输出 `Requirements: 0`、`DocSections: 1`。
...
**目标：** 用统一 `LanguageIndexBatch` 提取 Dart 文件、符号和测试；关系声明只来自 `.groundgraph/links.yaml`。Rust lightweight scanner 是默认 fallback；P7 之后可通过 Dart analyzer sidecar 获得 resolved AST 精度。

**实现范围：**

- 扫描配置中的 code paths，默认覆盖 `lib / test`。
- 提取 file、class、method、function、constructor、import、`test(...)`、`group(...)`。
- 可选 sidecar 通过 `GROUNDGRAPH_DART_ANALYZER=1` 启用，输出同一批量协议；失败时回退到 lightweight，不阻断索引。
- 输出 `LanguageIndexBatch`，由 Core 统一入库。
- 建立 symbol range 和 parent-child hierarchy。
- 索引 `.groundgraph/links.yaml`，建立 `Documents`、`DeclaresImplementation`、`DeclaresVerification` 边。
...
- `groundgraph slice <confirmed-business-logic-id>`。
...
- 断言 `groundgraph impact --base main` 输出 changed symbol、affected requirement、affected doc、linked test。
...
- Impact Review Check。
- `groundgraph context REQ-ID --json` 输出 docs、implementation、linked_tests、risks、files_to_read、tests_to_run。
...
- 新增 LogicConfidence report：
  ...
  - `unknown`：证据不足。
- 新增 candidate 输出到 `.groundgraph/candidates/`。
- 新增可选 `.groundgraph/requirements.yaml`，用于保存人工确认后的 AI 业务逻辑草案，尤其是没有业务文档时的外置需求描述。
- 新增 `groundgraph ask`，根据 evidence 生成需要用户回答的问题。
- AI 只生成 candidate / questions，不直接写 confirmed graph。
- 禁止用规则匹配生成业务关联。

**TDD 起点：**

- 有业务文档、实现和测试但无 links 时，AI 从 DocSection 与代码/测试事实生成 candidate，经确认后写入 `.groundgraph/links.yaml`。
...
- 用户确认 candidate 后，只更新 `.groundgraph/links.yaml` 或 `.groundgraph/requirements.yaml`。
...
6. **CLI e2e tests**
   - 使用 `assert_cmd` 和临时 fixture 仓库执行真实 `groundgraph` 命令。
...
- `groundgraph init` 在空目录成功生成 `.groundgraph.yaml`、`.groundgraph/graph.db`。
- 重复运行 `groundgraph init` 不破坏已有配置。
- SQLite migration 幂等。
- `groundgraph export --format jsonl` 对空图也能输出合法文件。
...
- `.groundgraph/links.yaml` 能把 `REQ-WATERMARK-001` 连到实现类和测试。
- method 改动能映射到 method symbol。
- method 改动能通过 parent class 和 links manifest 回溯到 Requirement。

### MVP-3 验收

- `groundgraph slice REQ-WATERMARK-001` 能稳定输出需求文档、实现类、测试。
- 无 linked test 时输出 warning/risk。
- 默认不把 imports 纳入 feature slice。

### MVP-4 验收

- 修改实现类后，`groundgraph impact --base <baseline>` 输出 changed symbol、affected requirement、affected doc、linked test。
...
- `groundgraph check` 能区分 error、warning、info。
- broken link 是 error。
- orphan requirement 和 missing linked test 是 warning。
- `groundgraph context REQ-WATERMARK-001 --json` 输出可反序列化 JSON。
...
- CLI e2e 覆盖 PRD 中 6 条命令：
  ...
  - `groundgraph impact --base <baseline>`
  - `groundgraph check`
...
- JSONL export、stable node ID、edge assertion schema、`.groundgraph.yaml` 保持向后兼容。
...
- 增加 `groundgraph link list`。
- 增加 `groundgraph link edit`。
- 增加 `groundgraph link remove`。
- 增加 `groundgraph link check`，只检查 `.groundgraph/links.yaml` 解析和节点可达性。
- 验收：人工可以修正已确认关系，但不是主要建链入口。

### P3：外置 Requirement Registry

- 增加 `.groundgraph/requirements.yaml`。
...
- 增加 `groundgraph ask`，输出带 evidence 的澄清问题。
- 增加 `.groundgraph/candidates/` 保存候选关系和候选需求。
- 增加 `groundgraph accept-candidate`，用户确认后写入 `.groundgraph/links.yaml` 或 `.groundgraph/requirements.yaml`。
...
落地内容（CLI 子命令 `groundgraph graph`）：
...
- `--format html`：生成 `.groundgraph/export/graph.html`，自包含 HTML+CSS+JSON+JS，零远程依赖（无 `https://`/`http://`/CDN）。lane 列布局 Documents/Business/Code/Tests/Risks；layer 分别用 `layer-fact`/`layer-confirmed`/`layer-candidate`/`layer-risk` CSS 类区分；支持搜索、focus、图层开关、节点/边详情面板；SVG 贝塞尔在 resize/filter 后重算。
- 通用旗标：`--focus <id>`（支持 `REQ-*` 稳定键、module path 或完整 artifact id）、`--max-nodes N`（截断时附 `graph_truncated` finding，确保保留 focus 与 confirmed 节点）、`--include-risks`（基于 `compute_checks` 注入 risk findings）、`--include-candidates`（读取 `.groundgraph/candidates/business_logic.yaml` 的候选层）。
...
- `EdgeSource::ExternalManifest` → layer=`confirmed`（来自 `.groundgraph/links.yaml`）。
- `EdgeCertainty::Fact`（filesystem/parser 提供）→ layer=`fact`。
- 候选：来自 `.groundgraph/candidates/business_logic.yaml`，作为 `GraphLayer::Candidate` 展示，不参与 confirmed slice。
...
- `business`：仅渲染 requirement + 其单跳邻居（docs/impl/test），无 requirement 时显式空状态提示 `groundgraph connect propose`。
- `focus`：单节点邻域图，复用现有 focus 解析。

CLI：

```bash
groundgraph graph --format html --view overview      # 默认
groundgraph graph --format html --view code --focus lib/features/editor
groundgraph graph --format html --view business
```
...
- `GROUNDGRAPH_DART_ANALYZER=1` 启用 sidecar；`GROUNDGRAPH_DART_ANALYZER_BIN` 可指定入口。
...
- `.groundgraph/candidates/business_logic.yaml` 作为候选输入。
...
- 用户确认：用户只需要判断自然语言描述是否符合真实产品设计，不需要手写 `.groundgraph/links.yaml`，也不需要人工初始标注代码关系。
- 系统写入：被用户确认的候选才能转成外置 confirmed requirement/link；未确认候选继续停留在 `.groundgraph/candidates/`。
...
- 人工确认后才写入 `.groundgraph/links.yaml` 或 `.groundgraph/requirements.yaml`。
...
**目标：** 提供独立命令 `groundgraph dead-code`，把代码图变成可用的“死代码可信度报告”而不是删除建议。报告必须给出置信度和理由，不替操作员决策。

**已落地：**

- 新 CLI：`groundgraph dead-code [--json] [--min-confidence high|medium|low] [--include-tests]`。
...
- `.groundgraph.yaml` 新增 `dead_code` 配置（默认 `lib/main.dart` 入口、`**/*.g.dart` / `**/*.freezed.dart` / `**/generated/**` / Flutter `app_localizations*.dart` 等 ignore、可选 `public_api_roots`）。
...
- 任何分析边界（入口点、ignore、public API）都只通过 `.groundgraph.yaml` 表达。
...
- 新 crate `groundgraph-mcp`，提供 `groundgraph-mcp` 二进制：stdio 上的换行分隔 JSON-RPC 2.0，MCP 协议版本 `2024-11-05`。
- 启动顺序：`--repo-root <path>` → `GROUNDGRAPH_REPO_ROOT` → 当前目录；每条 `tools/call` 还可在 `arguments` 里覆写 `repo_root`，便于 Agent 在多仓库间复用同一进程。
...
- 测试：`dispatcher_*` 直接覆盖 `initialize / tools.list / 未知方法 / 未知工具 / 工具失败`；`end_to_end_initialize_list_search_against_watermark_fixture` 通过真实 `groundgraph-mcp` 二进制 + watermark fixture 走通 `initialize → tools/list → tools/call(search_graph) → tools/call(dead_code)`，断言每一步的 JSON-RPC 信封、tool 内嵌 JSON 与 `isError` 标志。
...
- 不在代码中加任何注解；MCP 工具与 HTML 阅读器都只读 `.groundgraph.yaml` + graph.db + `business_logic.yaml`。
...
**目标：** 把 GroundGraph 的事实图从 Dart 扩出 Swift / Go 两条边，按规划用上游 LSP（`sourcekit-lsp`、`gopls`）当 sidecar，不自己写 parser，统一图模型由语言侧 Adapter 产出节点和边。

**已落地：**

- `groundgraph-core` 新增 11 个语言节点：`SwiftClass / SwiftStruct / SwiftEnum / SwiftProtocol / SwiftMethod / SwiftFunction / SwiftInitializer` 和 `GoStruct / GoInterface / GoMethod / GoFunction`，全部沿用 `language_prefix_kind` 约定，`as_str` / serde 序列化与 store 解码同步扩。
...
- 新增 `swift_indexer` + `go_indexer`：分别映射 sourcekit-lsp / gopls 的 `documentSymbol` 到 GroundGraph 节点类型；qualified name 采用 `<file>::Type.Member` 形式，跨语言保持稳定。`GROUNDGRAPH_SWIFT_LSP_BIN` / `GROUNDGRAPH_GO_LSP_BIN` 可覆盖二进制路径，`.groundgraph.yaml` 的 `swift / go.lsp_command` 同样生效。
...
- CLI / MCP 适配：`groundgraph search --kind swift_method,go_struct,...` 直接生效；MCP `search_graph` / `get_subgraph` 的 `kinds` 数组支持全部新 kind 别名；`graph` 列分组与「业务噪声排序」也同步把 swift/go 的方法纳入降权名单（`build / dispose / toString` 等）。
...
- 不写自家 parser；语言事实由上游官方 LSP 服务器（`sourcekit-lsp` / `gopls`）产出，GroundGraph 只做结构化吸纳。
...
- **CLI 输出 Swift / Go 段**：`groundgraph index` 的 `print_result` 拆出 `format_result`，在配置启用 `swift.enabled` / `go.enabled` 时分别打印 files / symbols / resolver_used / skip_reason；五条新 `format_result` 单测同时覆盖「未启用」「Indexed」「Skipped 含 PATH 提示」三种渲染分支。
...
- LSP 调用仍然是 best-effort：任何一条 `prepareCallHierarchy` / `outgoingCalls` / `references` 错误都不会让 `groundgraph index` 失败，只会进 `stats.skip_reason` 让 CLI 渲染出来。
...
- `crates/groundgraph-engine/tests/impact.rs::impact_propagates_via_calls_and_references_to_callers_and_tests` — depth=0 baseline + depth=2 双跳，反向 BFS 覆盖 `TestCase` 升级到 `linked_tests`。
- `crates/groundgraph-engine/tests/slice.rs::slicing_fans_out_via_calls_and_references_from_implementations` — depth=0 退回 manifest-only / depth=1 单跳 / depth=2 双跳三条分支。
- `crates/groundgraph-engine/src/dead_code.rs::tests::swift_lsp_calls_participate_in_dead_code_reachability` — 回归测，锁定按 kind 接入而非按 indexer 白名单。
- `crates/groundgraph-mcp/src/tools/get_subgraph.rs::tests` — `parse_resolvers` 合法 / 非法形状、`resolver_allowed` 空集合 vs 非空集合的过滤语义。
- `crates/groundgraph-cli/src/commands/graph_mermaid.rs` 新增 `render_parts_renders_minimal_diagram_and_drops_dangling_edges` / `render_parts_emits_empty_subgraph_comment_when_no_nodes`，锁定核心渲染器的 dangling edge 容错与空图退化。
...
- `lsp_indexer.rs::probe_call_hierarchy_and_references` 改成：解析到的 `outgoing.to` 仅用于 `resolver.resolve` 找 GroundGraph 侧的 callee artifact，`ReferenceEdge` 的 `source_file` / `line` 改写为 caller 自身的 `sym.file_rel` + `outgoing.from_ranges[0].line`（如果 fromRanges 为空则退到 `sym.selection_line`，仍然在 caller 文件而非 callee 声明位置）。`(from, to, kind)` 的 dedup 集合保证多次调用同一 callee 只生成一条边，第一处调用点成为权威 evidence。
...
- `crates/groundgraph-engine/src/lsp_client.rs::tests::parse_outgoing_calls_returns_from_ranges_alongside_callee` — 纯解析层锁定 `fromRanges`。
- `crates/groundgraph-engine/tests/lsp_indexers.rs::swift_indexer_emits_*`（`#[ignore]`，`--include-ignored` 才跑）— 真实 sourcekit-lsp 路径下，如果产出 Swift `Calls` 边，则 evidence 必须落在 caller 文件；Calls 为空会作为 opt-in smoke 的软提示输出。
- `crates/groundgraph-engine/tests/impact.rs::impact_records_real_edges_for_calls_and_requirement_anchors` — `impact_edges` 必须包含 `(class → req, declares_implementation)`（而非 cross-product 到改动叶子）、`(test → req, declares_verification)`、`(caller → callee, calls)`，且 BFS 双跳能补出 `(t → b)` 与 `(b → a)`。
...
- 真实 LSP 集成测试转为 opt-in 后，沙箱 CI 默认零 LSP 依赖；任何机器没有 `sourcekit-lsp` / `gopls` 都能跑全量 `cargo test --workspace`，与「`groundgraph index` 的 LSP 缺失即跳过」的运行时承诺一致。
- Calls evidence 改用 `fromRanges` 后，所有上游消费（图详情 / Mermaid / Impact / 审计快照）的 file:line 都指向调用点。LSP 端如果未来还报 `to.uri` 与 callee 不一致的 bug，也不会污染 GroundGraph 自己的 evidence 列。

## P16 Python 适配（LSP-first + AST 补强）（已落地）

P15 闭环 Swift / Go 后，下一道短板是 Python 这条 GroundGraph 用户最常碰到的语言。Python 的动态特性使得「纯 LSP 索引」既无法覆盖 imports / pytest，也无法把 `Calls` 当成强事实，因此 P16 直接把 LSP 与 AST 拆成两条互补通路并强制全部跑过一遍，避免在 CI / 沙箱里只有 LSP 时再静默吞掉 import / 测试事实。

**实现要点：**

- `groundgraph-core::NodeKind` 新增 `PythonModule / PythonClass / PythonFunction / PythonMethod`，并接入 `column_for / kind_rank / is_code_kind / is_implementation_kind / collect_orphan_symbols / search.default_search_kinds / search.kind_to_zh / MCP & CLI 的 --kind 解析 / store 字符串↔kind 映射`。CLI / MCP 全部接受 `python_function / py_function / pyfunc` 等别名，方便 Agent 与人手都能写。
- `crates/groundgraph-engine/src/python_ast.rs` 是纯 Rust、无依赖的 Python AST 扫描器（indentation + 行级 token）：识别 `class`、`def`、`async def`、`import` / `from ... import ...`、`@pytest.fixture`、`@pytest.mark.parametrize`、`Test*` 类、`test_*` 函数；同时跟踪三引号 docstring 块避免误把 docstring 里的 `def` 当成符号。
- `crates/groundgraph-engine/src/python_indexer.rs` 是入口适配器：复用 `LspProfile` 通路驱动 `pyright-langserver` / `basedpyright-langserver` / `pylsp`，按 `GROUNDGRAPH_PYTHON_LSP_BIN → python.lsp_command → <repo>/.venv/bin/{basedpyright,pyright,pylsp}-langserver → PATH` 顺序自动发现，并强制带 `--stdio`。env / `python.lsp_command` 是「权威覆盖」：操作者点名某个 binary 但不存在时直接降级到 AST，不再静默回退到 PATH 上的同名工具。
...
- `EngineConfig` 新增 `python: LanguageAdapterConfig`，默认 `enabled: false`，shape 与 swift / go 对齐。CLI `groundgraph index` 输出新增 `Python index:` 段，列出 files / Symbols / TestCases / Imports / Resolver / LSP skipped reason，方便操作者一眼看清 LSP 是否真的接上。
...
P16 落地后我们在 `/Users/qjs/Code/Projects/atagent`（典型 FastAPI 后端，~165 py 文件，无 `__init__.py` 的 `backend/` src layout）做了非侵入式验证，只在仓库根写入临时 `.groundgraph/` 工作区并在结束时清理，未触碰任何业务源码。验证结论：
...
- `groundgraph search "list_blocks"` 在 CLI 文本输出里直接显示 `框架角色: fastapi_route`，AI Agent 与人工读图都能立刻看清"这是被 FastAPI 反射调用的 route"。
- 验证完成后已清理 `/Users/qjs/Code/Projects/atagent/.groundgraph` 工作区，目标仓库代码零修改。
...
- 新增 CLI `groundgraph similar [--node SYMBOL_ID] [--min-tokens N] [--min-cluster-size N] [--format text|json]`。文本输出按 normalized_token_count 降序排列簇，方便操作者先看「最大段重复」；JSON 通路供 MCP / 上层 Agent 直接消费。
...
- `groundgraph similar` 扫描 1043 函数 / 方法（跳过 63 个 body < 12 tokens 的 trivial），输出 **107 个结构重复簇**；过滤掉 `vscode-copilot-chat/...` 第三方 fixture 后仍有 **96 个 backend/ 内部纯净簇**，全部为真实重构候选。
...
- `groundgraph similar --node ... --format json` 模式输出符合 schema_version=1 的 JSON，可直接灌入 MCP / 自动化流水线。
- 验证完成后已清理 `/Users/qjs/Code/Projects/atagent/.groundgraph` 工作区，目标仓库代码零修改。
...
- 修正 `.groundgraph.yaml`：Python 索引器读 `python.paths`，不读 `roots.code`。确认后只索引 backend/（165 文件，1224 符号），不再混入 vscode-copilot-chat fixture。
- `groundgraph similar --mode all` 输出 **96 exact + 55 near = 151 簇**。
...
`crates/groundgraph-engine/src/edge_confidence.rs` 把 `(EdgeKind, EdgeSource, EdgeCertainty, EdgeStatus, indexer)` 五元组折叠成 `EdgeConfidence::{High, Medium, Low}`。`GraphEdge` 序列化时多写一个 `evidence_quality: "high"|"medium"|"low"` 字段，consumers（人 / AI / MCP）不需要重新解析 provenance 元组就能筛选。
...
## P19 — `groundgraph select-tests`（已落地）

`crates/groundgraph-engine/src/test_selection.rs` 复用 `compute_impact_with_policy` 取得 changed_files / changed_symbols / propagated_symbols，再按三条规则选测试：
...
CLI：`groundgraph select-tests --base main [--head HEAD] [--include-deps] [--max-depth N] [--format text|json]`。

atagent HEAD~1..HEAD 验证：6 改动文件 → 6 改动符号 → 选出 85 个测试（2 high + 83 medium）。

## P19 — `groundgraph features`（已落地）

`crates/groundgraph-engine/src/feature_map.rs` — 两阶段启发式聚类：
...
CLI：`groundgraph features [--max-clusters 20] [--max-depth 3] [--min-cluster-size 3] [--format text|json]`。
...
## P19 — `groundgraph graph-diff`（已落地）

`crates/groundgraph-engine/src/graph_diff.rs` 比对两份 `.groundgraph/graph.db` 快照（典型用法：CI 把 base / head 的 graph.db 都缓存下来），输出：

- `nodes_added` / `nodes_removed` / `nodes_kind_changed`
- `edges_added` / `edges_removed` / `edges_status_changed`（捕获 confirmed → deprecated 等转变）

CLI：`groundgraph graph-diff --base-db <path> --head-db <path> [--format text|json]`。
...
## P19 — `groundgraph questions`（已落地）

`crates/groundgraph-engine/src/questions.rs` 把"代码图里需要人 / Agent 确认的事实"做成可读的问题列表。MVP 四个类别：
...
每条问题都附带 `artifact_id` 与 `path`，AI 可以直接喂给 `groundgraph graph --focus <id>` 取上下文。

CLI：`groundgraph questions [--max-per-category 20] [--format text|json]`。
...
P16/P17/P18 把 Python 通路打通到框架装饰器与相似聚类后，仍然遗留五道"声明已收口但用户跑起来会立刻碰壁"的真实门：(1) TypeScript / Java 实际还没有进图；(2) Python opt-in LSP smoke 在 shebang 损坏的 `pylsp` 上不会软跳过；(3) `groundgraph questions` 的 pending candidate 检测从 `graph.db` 节点而不是 `.groundgraph/candidates/business_logic.yaml` 读，真实候选完全不会出现在报告里；(4) `graph-diff` 文档承诺了 candidate diff，实现里只有 nodes / edges；(5) `is_code_symbol` 在 `questions.rs` 被复制了一份不完整版本（漏 `SwiftInitializer / SwiftEnum / SwiftProtocol / GoInterface / PythonModule`），证明跨语言一致性已经在漂移。P20 同时解掉这五道。

**实现要点：**

- **统一语言能力层**：新模块 `crates/groundgraph-core/src/language_traits.rs` 提供 `language_of(kind) / family_of(kind) / is_code_symbol / is_callable / is_type / is_module_or_file / is_test / similarity_supported / default_dead_code_reason / search_aliases`。`questions.rs` / `dead_code.rs` / `feature_map.rs` / `slice.rs` / `search.rs` / `groundgraph-mcp::tools::parse_node_kind` / `groundgraph-store::repositories::node_from_row` 全部改为调用这个模块，不再各自维护 `match` 分支。`language_traits::tests` 包含 41 项 `ALL_KINDS` 全枚举矩阵测试，新加语言时漏掉任何一个 family/predicate 都会立刻爆红。
...
- **TypeScript adapter**（`typescript_indexer.rs` + `typescript_ast.rs`）：
  - LSP-first via `typescript-language-server --stdio`（自动嗅 `node_modules/.bin/typescript-language-server` → PATH，`GROUNDGRAPH_TYPESCRIPT_LSP_BIN` 覆盖），失败原因写进 `sidecar_skip_reason`。
...
- **Java adapter**（`java_indexer.rs` + `java_ast.rs`）：
  - LSP-first via `jdtls`（`GROUNDGRAPH_JAVA_LSP_BIN` 覆盖），AST 补强始终运行。
...
- **`groundgraph questions` 改为 YAML 优先**：pending candidate 检测从 `.groundgraph/candidates/business_logic.yaml`（经 `load_business_candidates`）加载，按 `review_status ∈ { pending, needs_changes, missing }` 报 warn 级问题；并保留对 `BusinessCandidate` store 节点的回退路径以兼容老仓库。`questions.rs` 内部不再维护 `is_code_symbol` 副本，全部走 `language_traits::is_code_symbol`。
- **`groundgraph graph-diff` 补齐 candidate diff**：`GraphDiffOptions` 增加 `base_repo_root / head_repo_root`；提供时 `diff_candidates_into` 从两侧仓库加载 candidate YAML 后输出 `candidates_added / candidates_removed / candidates_status_changed`，并体现在 `GraphDiffStats` 与 CLI 文本 / JSON 报告。CLI 新增 `--base-root / --head-root` flag。
...
- `cargo test -p groundgraph-engine --test lsp_indexers -- --include-ignored`：10 passed / 0 failed（本机 Swift + Go + TS 真启动，Python + Java soft-skip）
- `cargo test -p groundgraph-cli --bin groundgraph commands::index`：11 passed（含 4 个新 TS/Java 渲染测试）
- `dart test`（`tool/groundgraph_dart_analyzer/`）：6 passed

## P20 收口补丁（第二批：统一 LSP probe）

第一批收口走完后还有两个尾巴：

- 复核者跑完整命令 `cargo test -p groundgraph-engine --test lsp_indexers -- --include-ignored` 时 Swift LSP smoke 仍 hard fail，原因是 `sourcekit-lsp` 在他的机器上「PATH 上可见但 stdio 启动后立刻 `SOURCEKITD FATAL ERROR: Service is invalid`」，而第一批的 soft-skip 只覆盖 `resolver_used != "swift_lsp"`，没有覆盖 `index_swift` 直接返回 `Err` 的崩溃路径。
- 第一批中只有 Python 改成了「真启动一次 `--help` 才算可用」，Swift / TypeScript / Java / Go 仍然只检查二进制是否在 PATH 上，导致同样的失败模式还会在别的语言上复发。

第二批一次性把 LSP probe 拉到统一层。

1. **新增 `crates/groundgraph-engine/src/lsp_probe.rs`**：提供 `probe_lsp_command(command, args, timeout) -> ProbeReport`。`Runnable` 当且仅当进程在 `timeout`（默认 1500ms）内退出 0 且 stderr 不命中 `broken stub` 标记（`bad interpreter` / `no such file or directory` / `no module named` / `cannot execute` / `command not found` / `SOURCEKITD FATAL ERROR` / `could not load` / `no java runtime` / `JAVA_HOME is not set` / `node: command not found`）。模块内带 8 个 TDD 单测覆盖 ok / missing / broken-shebang / timeout / non-zero-exit / sourcekitd / 缺 JVM / 干净帮助文本。
2. **Python adapter 改为薄壳**：`crates/groundgraph-engine/src/python_indexer.rs::runs_ok` 从 60 行手写 spawn-and-timeout 缩成 8 行，转调 `crate::lsp_probe::probe_lsp_command(…)`。`python_lsp_available_rejects_binary_with_broken_shebang` 等 3 个回归测试仍绿。
...
4. **opt-in 测试再加一层 Err → soft-skip**：`crates/groundgraph-engine/tests/lsp_indexers.rs` 中 Swift / Python / Go / TypeScript / Java 五个 opt-in smoke 把 `index_<lang>(&mut store, &opts).expect(...)` 改成 `match … { Err(e) => { eprintln!("soft-skip … `index_<lang>` returned Err ({e}); …"); return; } }`。即便 probe 误判可用、stdio 真启动后立刻崩溃，测试也会 soft-skip 而不是 panic。
...
- 新增：`crates/groundgraph-engine/src/lsp_probe.rs`
- 修改：`crates/groundgraph-engine/src/{lib.rs, python_indexer.rs, swift_indexer.rs, go_indexer.rs, typescript_indexer.rs, java_indexer.rs}`、`crates/groundgraph-engine/tests/lsp_indexers.rs`

## v0.2.0 正式收口（release artifact + 真实仓库扫描）

P20 + 小批次收口完成后正式打 0.2.0：

- `workspace.package.version = "0.2.0"`，`tool/groundgraph_dart_analyzer/pubspec.yaml::version = 0.2.0`，CLI `--version` 自检：`groundgraph 0.2.0`。
- `bash scripts/release_macos_universal.sh`：生成 `dist/groundgraph-0.2.0-macos-universal.tar.gz`（含 arm64 + x86_64 lipo 通用二进制 + Dart sidecar 源码 + AI Skill），`bash scripts/validate_macos_package.sh dist/groundgraph-0.2.0-macos-universal.tar.gz` 通过；
...
- `packaging/skills/groundgraph/SKILL.md`：补 `java_enum` 独立语义说明。
...
- `cargo test --workspace`：581 passed / 0 failed / 5 opt-in LSP smokes ignored
- `dart test`（`tool/groundgraph_dart_analyzer/`）：6 passed
- `bash scripts/validate_macos_package.sh dist/groundgraph-0.2.0-macos-universal.tar.gz`：通过
- 四个仓 scratch-scan 全部生成 `report.md`；目标仓 `.groundgraph/` 时间戳全部早于本次扫描（pixcraft-landing 与 vub 根本没有 `.groundgraph/`），确认非侵入。
...
| Phase | 主体改动 | 关键新代码 / 测试 |
|-------|----------|---------------------|
| 1 | 新模块 `confidence_view`：`EdgeQualityScope` (`Usage` / `SearchRanking`) + `EdgeQualitySummary` + 纯函数 `summarize_edges` + store 查询 `inbound_edge_quality` / `outbound_edge_quality` + 邻接收集 `neighbors_of(cap)` | `crates/groundgraph-engine/src/confidence_view.rs`（20 单测） |
| 2 | dead-code 在 `classify(...)` 末尾给"非 Low 桶但 inbound usage 边全是 low-tier"的候选追加 reason；`DeadCodeReport` 引入 `warnings: Vec<String>`（`skip_serializing_if = "Vec::is_empty"`） | `crates/groundgraph-engine/src/dead_code.rs`（+4 单测） |
| 3 | search 在 `matches.sort_by` 之前插入 Pass A（出边 evidence_quality=high → +30 score）+ Pass B（邻接 cluster → 最多 +20 score，reason 最多列 2 个名字 + "等"）；`SearchResult` 引入同型 `warnings` 字段，engine 端**任何 stderr 都改 push warnings** | `crates/groundgraph-engine/src/search.rs`（+7 单测、回归修 `tests/p5_search_golden.rs::p5_file_line_input_resolves_to_enclosing_symbol`） |
| 4 | CLI 渲染：`groundgraph search` / `groundgraph dead-code` 人类输出末尾增加 `== Warnings ==` 段（空列表完全静默）；MCP `search_graph` / `dead_code` tool 的 JSON-RPC 输出自然透传 warnings（通过 round-trip 测试锁死） | `crates/groundgraph-cli/src/commands/{search,dead_code}.rs`、`crates/groundgraph-mcp/src/tools/{search_graph,dead_code}.rs`（+4 CLI 单测、+4 MCP 单测） |
...
- ~~LSP 调用链大面积失败时（如 sourcekit-lsp 在大型 Swift 仓上
  ...
  `skip_reason`，最终 `groundgraph index` 把单行 **185 KB** 直接 `print!`
...
CodeGraph 对标实测暴露了一条**根本路径错误**：GroundGraph 的多语言全部押在
...
与速度上完胜。P21 不再背历史包袱，引入 GroundGraph 的第一个 **tree-sitter 广度
后端**，先把"能索引自身"这条最难堪的缺口堵上。

**实现要点：**

- **依赖**：`tree-sitter 0.26` + `tree-sitter-rust 0.24` 直接链入 `groundgraph-engine`
  （C 语法在 build 期编译，运行期零外部依赖、完全确定性）。
- **`crates/groundgraph-engine/src/rust_treesitter.rs`**：纯函数 `scan(&str) -> RustScan`
  ...
  对抗恶意深嵌套不爆栈。
- **`crates/groundgraph-engine/src/rust_indexer.rs`**：`index_rust` 走 walkdir 发现
...
`rust.enabled=true, paths:[crates]` → `groundgraph index` →
...
- `cargo test --workspace`：全绿（`groundgraph-engine` lib **288 passed**，含 15 项 Rust
...
**架构：一个驱动，多种语言（`crates/groundgraph-engine/src/treesitter.rs`）**

- **`LangSpec` 数据驱动**：每门语言只提供 `language_id` + `grammar`（编译期链入的 tree-sitter 语法）+ `extensions` / `skip_dirs` / `separator` + `func_kind` / `method_kind`，再加一把小函数指针钩子，把该语法的 node kind 映射到 GroundGraph `NodeKind`。用 `fn` 指针而非 trait object，`LangSpec` 是零分配的 `static`。
...
- **CLI / MCP / search**：`groundgraph index` 输出新增 `Tree-sitter index:` 段，逐语言打印 files / symbols / imports / resolver；CLI `match_kind` 与 MCP `parse_node_kind` 补齐 `c_*` / `cpp_*` 别名（`cfn` / `cpp_ns` 等）；`search::default_search_kinds` 纳入 C/C++ 变体。
...
- `cargo test --workspace`：全绿——`groundgraph-core` 30 passed；`groundgraph-engine` lib **313 passed**（含六语言单元 + 逐 spec 属性测试）；`p22_treesitter_multilang` 2 passed、`p21_rust_self_host` 1 passed；CLI / MCP / store 全绿；5 个 opt-in LSP smoke ignored。
- `dart test`（`tool/groundgraph_dart_analyzer/`）：6 passed。
...
- **Dart（P23.6，#4，最高风险）**：新增 `tree-sitter-dart`，`dart_treesitter.rs` 作结构来源；analyzer/sidecar 降为 Tier-3 语义/calls（`reads_provider`/`navigates_to`/… + Calls + `DartProvider` 框架节点），沿用 `dart_*::` 旧 id 方案使 overlay 零翻译绑定；退役 `groundgraph-lang-dart` 结构职责；**pixcraft 等 golden 保持全绿**。
...
1. **doc 索引吞掉 `node_modules`（图体积虚高 2.4×）：** `index_docs` 的 `walkdir` **无目录剪枝**，会下钻 `node_modules`/`.git`/`target` 等——本仓 `docs/` 下 vendored 的 CodeGraph `node_modules` 让 DocSection 虚增到 **1518**（绝大多数是第三方 README）。新增 `is_pruned_dir`（对齐 tree-sitter 驱动的 `skip_dirs`：`node_modules`/`.git`/`target`/`dist`/`.venv`/`.groundgraph`/… 一律不下钻），DocSection **1518→620、node_modules 段 0**；新增 `docs_walk_prunes_vendor_directories` 回归。
...
**P23.15 第四轮 dogfood（Dart 大仓真实扫描 + 死代码假阳收口；analyzer 自洽化）：** 用发版二进制 + AOT 编译的 analyzer sidecar 对多个真实 Flutter 仓做非侵入扫描（图写入各仓 `.groundgraph/`，临时仓用毕即清理），以 `dead-code` 的 **high 置信候选数**为假阳指标，逐一 TDD 修复：
...
2. **AOT 编译的 sidecar 找不到 Dart SDK（生产快路径不可用）：** 文档允许把 `GROUNDGRAPH_DART_ANALYZER_BIN` 指向**编译后的二进制**（大仓比 `dart run` 显著快），但 analyzer 7.7.1 的 `getSdkPath()=dirname(dirname(Platform.resolvedExecutable))` 对 AOT exe 解析成 exe 自身目录（`/tmp/lib/_internal/…`），抛 `PathNotFoundException`，sidecar 整体 skip 回退 lightweight。修复：`walker.dart` 新增 `resolveSdkPath()`——`dart run` 下默认有效则返回 `null`（行为不变）；AOT 下依次用 `GROUNDGRAPH_DART_SDK`/`DART_SDK`、再在 `PATH` 上定位 `dart` 推导 SDK（兼容纯 Dart SDK 的 `<sdk>/bin/dart` 与 Flutter 的 `<flutter>/bin/cache/dart-sdk`），以 `sdk_library_metadata/libraries.dart` 存在性校验。`resolvedExecutable`/`environment` 注入便于测试；新增 5 项 `resolveSdkPath` 单测（含「默认 bogus 时经 PATH 复原」「`GROUNDGRAPH_DART_SDK` 覆盖优先」）。实测 AOT exe 对 turing：`ok:true`、20 456 symbols、stderr 空。
...
- `cargo test -p groundgraph-cli`：全绿（init/index/slice/impact/graph/check + graph mermaid overview/business 集成断言）。
- live dogfood（非侵入，写临时 `.groundgraph/`）：`index` 0 unresolved；`slice`/`impact`/`graph`/`check` 全部连通验证（见 P23.10）。
