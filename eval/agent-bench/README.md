# Agent code-lookup benchmark: grep vs groundgraph

Measures whether giving a coding agent the **groundgraph** code graph changes its
...
- **Two arms, one variable:** both arms get the *same* model, workspace and task
  ...
  - `grep` — "use only ripgrep/grep + file reads; do not use groundgraph"
  - `groundgraph` — "you have the groundgraph CLI (search/trace/impact/...); prefer it"
  The recorded tool calls let `score.py` verify each arm stayed in its lane
  (`purity` column: a `grep` run that touched groundgraph is flagged `LEAK`).
- **Independent ground truth:** answer keys in `tasks.json` are hand-verified
  from source + ripgrep, **never derived from GroundGraph's own graph** — otherwise
...
- **t2 is the only decisive win.** The `grep` arm could not converge on the true
  ...
  definition + 4 doc-comment mentions of `kill_tree`. The `groundgraph` arm
...
- **t1 / t3 / t4 / t5 all tie at 1.00.** Definitional lookup, cross-file caller
  ...
  trivial graph sizes. `groundgraph` matched accuracy but on t4/t5 often spent
  *more* tool calls and tokens exploring.
- **A groundgraph rough edge t5 surfaced:** `groundgraph dead-code` defaults to
...
- **The scorer had to be hardened (self-fix).** The first t5 scoring falsely gave
  one `groundgraph` run 0.11: the agent glued prose onto its answer line
...
- **t6 (business logic, MetaQuant) also ties at 1.00 — and exposed a groundgraph
  default bug.** Both arms perfectly enumerated the 17-fn guardrail closure
  (grep reads the 530-line file and traces; groundgraph runs `trace`). But the
  groundgraph arm was *not* cheaper: `groundgraph trace <symbol>` defaulted to 6
...
- **Implication for the next iteration:** trivial fixtures don't separate the
  ...
  *accuracy* gap you need **scale**: deep/wide call closures (10+ hops, fan-out),
  ...
  infeasible. The honest thesis so far is narrow: *groundgraph wins when the answer
...
```bash
SS=/Users/qjs/Code/Projects/groundgraph
git -C "$SS" worktree add --detach /Users/qjs/Code/Projects/groundgraph-bench/wt HEAD
( cd /Users/qjs/Code/Projects/groundgraph-bench/wt && groundgraph index )   # builds .groundgraph/graph.db

# t5 only: inject the dead-code fixture, print the compiler oracle, reindex
./setup_fixture.sh /Users/qjs/Code/Projects/groundgraph-bench/wt
```

`groundgraph` must be on `PATH` (`cargo install --path crates/groundgraph-cli`).
...
git -C "$MQ" worktree add --detach /Users/qjs/Code/Projects/groundgraph-bench/mq "$(git -C "$MQ" rev-parse HEAD)"
cp -R "$MQ/.groundgraph" "$MQ/.groundgraph.yaml" /Users/qjs/Code/Projects/groundgraph-bench/mq/   # graph.db stores RELATIVE paths → portable
```
...
python3 run.py                        # all tasks x {grep,groundgraph} x 1 seed
...
- **Availability ≠ usage.** The benchmark conflates "does the agent reach for
  groundgraph" with "is groundgraph better once used". The `purity`/`spec` columns
...
- **v1 benchmarks the groundgraph CLI**, not the MCP server integration. A v2 can
  attach `groundgraph-mcp` via a per-arm `.cursor/mcp.json` to test the real agent
  wiring.
