...
//!
//! **#65 (env data race).** `SPECSLICE_DART_ANALYZER*` are process-global, and
...
//! `cargo test` output) and, when `SPECSLICE_GOLDEN_REQUIRED` is set, turns the
//! missing toolchain into a hard failure so CI can opt into enforcement.
...
///
/// Sets `SPECSLICE_DART_ANALYZER=1` and `SPECSLICE_DART_ANALYZER_BIN=dart run
...
pub fn enable_dart_sidecar_env(sidecar_abs: &Path) {
    ...
    SET_ENV.call_once(|| {
        std::env::set_var("SPECSLICE_DART_ANALYZER", "1");
        std::env::set_var("SPECSLICE_DART_ANALYZER_BIN", &bin);
    });
}
...
/// `SPECSLICE_GOLDEN_REQUIRED`) or prints a *visible* skip to stdout and
...
pub fn dart_golden_ready(available: bool, ctx: &str) -> bool {
    ...
    assert!(
        std::env::var_os("SPECSLICE_GOLDEN_REQUIRED").is_none(),
        "{ctx}: SPECSLICE_GOLDEN_REQUIRED is set but the Dart sidecar is unavailable \
         (missing `dart` on PATH or sidecar source) — the golden regression cannot run",
    );
    println!(
        "skipping {ctx}: Dart sidecar unavailable; set SPECSLICE_GOLDEN_REQUIRED=1 to enforce",
    );
    false
}
