...
//!
//! Validation methodology: every analytic claim SpecSlice makes ("this is
...
fn c_tagged_typedef_struct_is_referenced_via_typedef_name() {
    ...
    let _ = index(root);
    let store = specslice_store::Store::open(root.join(".specslice/graph.db")).unwrap();
...
fn init_detection_skips_deps_dir() {
    ...
    specslice_engine::init::init_repository(specslice_engine::init::InitOptions::new(root))
        .expect("init");
    let config = fs::read_to_string(root.join(".specslice.yaml")).unwrap();
    assert!(
