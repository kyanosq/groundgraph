...
| L4 自举（self-host） | `p21_rust_self_host.rs` | SpecSlice 索引自身工作区必须产出有意义的图 | `cargo test --test p21_rust_self_host` |
| L5 端到端 | `crates/specslice-cli/tests/*`、`crates/specslice-mcp/tests/protocol.rs` | CLI 人类输出 / JSON 合同 / MCP 协议 | `cargo test -p specslice-cli -p specslice-mcp` |
