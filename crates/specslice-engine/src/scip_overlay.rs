...
//! translated into our [`ArtifactId`](specslice_core::ArtifactId): every SCIP
...
use specslice_core::language_batch::{ReferenceEdge, SymbolRange};
...
pub struct ScipOverlayResult {
    /// `.scip` files ingested under `.specslice/scip/`.
...
pub fn ingest_scip_overlay(store: &mut Store, repo_root: &Path) -> Result<ScipOverlayResult> {
    let scip_files = collect_scip_files(&repo_root.join(".specslice").join("scip"));
