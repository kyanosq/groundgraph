...
/// `/Users/alice/Code/proj/.groundgraph/graph.db` or CI's
...
mod tests {
    ...
    fn test_server() -> (Server, tempfile::TempDir) {
        ...
        std::fs::write(dir.path().join(".groundgraph.yaml"), "{}").unwrap();
    ...
    fn redact_paths_replaces_repo_root_with_placeholder() {
        let root = Path::new("/Users/qjs/Code/Projects/groundgraph");
        let msg = "opening SQLite database at /Users/qjs/Code/Projects/groundgraph/.groundgraph/graph.db: unable to open";
        let out = redact_paths(root, msg);
        assert_eq!(
            out,
            "opening SQLite database at <repo-root>/.groundgraph/graph.db: unable to open"
        );
    ...
    fn redact_paths_masks_home_prefixes_outside_the_repo_root() {
        ...
        let ci = redact_paths(
            Path::new("/home/runner/work/groundgraph/groundgraph"),
