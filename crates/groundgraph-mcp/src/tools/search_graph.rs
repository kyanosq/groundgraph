...
use groundgraph_engine::run_search_with_store;
use groundgraph_engine::search::{SearchOptions, SearchQuery};
use groundgraph_engine::{SEARCH_DEFAULT_DEPTH, SEARCH_DEFAULT_LIMIT};
...
pub fn descriptor() -> ToolDescriptor {
    ToolDescriptor {
        name: "search_graph",
        description: "Search the GroundGraph code graph for matches plus a 1-hop subgraph. \
            ...
            `groundgraph search` but emits structured JSON only.",
...
#[cfg(test)]
mod tests {
    use groundgraph_engine::search::SearchSubgraph;
    use groundgraph_engine::SearchResult;
    ...
    /// MCP clients (and JSON consumers of `groundgraph search --json`)
