//! `groundgraph-mcp` — MCP stdio server entry point.
//!
...
//! 2. `GROUNDGRAPH_REPO_ROOT` environment variable.
...
const HELP: &str = "\
groundgraph-mcp — GroundGraph MCP (Model Context Protocol) stdio server

Usage: groundgraph-mcp [OPTIONS]
...
Options:
      --repo-root <PATH>  Default workspace root to analyse. Falls back to
                          $GROUNDGRAPH_REPO_ROOT, then the current directory.
                          Individual tool calls may override it per request.
  -h, --help              Print this help and exit.
  -V, --version           Print version and exit.

Environment:
  GROUNDGRAPH_REPO_ROOT   Default workspace root when --repo-root is absent.
";
...
pub fn main() -> ExitCode {
    match parse_invocation(env::args().skip(1)) {
        ...
        Invocation::Version => {
            println!("groundgraph-mcp {}", env!("CARGO_PKG_VERSION"));
            ExitCode::SUCCESS
        }
...
/// `$GROUNDGRAPH_REPO_ROOT` when set to a non-empty value.
fn env_repo_root() -> Option<PathBuf> {
    match env::var("GROUNDGRAPH_REPO_ROOT") {
...
mod tests {
    ...
    fn help_takes_precedence_over_a_preceding_repo_root() {
        // A probe like `groundgraph-mcp --repo-root /x --help` should print help,
        // not start serving.
