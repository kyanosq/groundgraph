//! Integration tests for the GroundGraph MCP server.
//!
...
//!    through the real `groundgraph-mcp` binary against the
...
fn packaged_skill_matches_mcp_launch_and_tool_contract() {
    // #99: the shipped skill is now the single source `skills/groundgraph/SKILL.md`
    ...
    .expect("read GroundGraph skill (skills/groundgraph/SKILL.md)");

    assert!(
        skill.contains("groundgraph-mcp --repo-root /path/to/repo"),
        "Skill must document the real MCP binary launch command"
    );
    assert!(
        !skill.contains("groundgraph --repo-root /path/to/repo mcp serve"),
        "Skill must not document a non-existent `groundgraph mcp serve` CLI command"
    );
    assert!(
    ...
    assert!(
        ...
        "Skill must not point users at a stale .groundgraph/config.yaml path"
    );
...
fn dispatcher_tool_error_is_returned_as_is_error_content() {
    ...
    let server = Server::new(PathBuf::from("/this/path/should/not/exist/groundgraph-mcp"));
    ...
    let text = value["result"]["content"][0]["text"]
        .as_str()
        .expect("text content");
    assert!(text.contains("groundgraph init") || text.contains("no GroundGraph workspace"));
}
...
fn end_to_end_initialize_list_search_against_watermark_fixture() {
    ...
    let out = Command::cargo_bin("groundgraph-mcp")
        .unwrap()
        .arg("--repo-root")
        .arg(tmp.path())
        .write_stdin(input)
