//! `specslice questions` — P19 clarifying-questions report.

use std::path::PathBuf;

use anyhow::{Context, Result};
use specslice_engine::questions::{analyze_questions, QuestionsOptions, QuestionsReport};
...
fn print_text(r: &QuestionsReport) {
    println!("SpecSlice questions (P19 · AI 澄清问题包)");
    ...
    println!();
    println!(
        "提示: 这份列表是给 AI 助手或评审者读的 — 它不会自动决定 accept / reject。每条问题都附带 artifact id，可以用 `specslice graph --focus <id>` 查看上下文。"
    );
}
