//! `specslice route-coverage` — HTTP route porting coverage (P26).
//!
...
//!
//! ```text
//! specslice route-coverage --source-db client/.specslice/graph.db \
//!                          --target-db server/.specslice/graph.db
//! specslice route-coverage --source-db a.db --target-db b.db --include-extra --json
//! ```

use std::path::PathBuf;

use anyhow::{Context, Result};
use specslice_engine::route_coverage::{
...
fn print_human(report: &RouteCoverageReport) {
    let s = &report.stats;
    println!(
        "SpecSlice 路由移植覆盖率账本 (schema v{} · 匹配键=末 {} 段)",
