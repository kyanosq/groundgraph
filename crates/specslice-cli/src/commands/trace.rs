//! `specslice trace` — 接口 → 整张图：把一个端点/符号的**完整下游链路**
...
use specslice_engine::trace::{run_trace, TraceOptions, TraceResult};
...
pub fn run(args: TraceRunArgs) -> Result<()> {
    ...
    specslice_engine::stats::set_metric("trace_nodes", result.nodes.len() as i64);
    specslice_engine::stats::set_metric("trace_edges", result.edges.len() as i64);
    specslice_engine::stats::set_metric("trace_tables", result.tables.len() as i64);
...
fn print_human(r: &TraceResult) {
    println!("SpecSlice trace（接口 → 整张图）");
    println!("查询: {}", r.query);
    if r.seeds.is_empty() {
        println!("\n(无命中：换个符号名，或先 `specslice index`)");
        return;
    }
