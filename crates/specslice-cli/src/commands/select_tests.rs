//! `specslice select-tests` — P19 test selection.
//!
...
use specslice_engine::test_selection::{
...
fn print_text(report: &TestSelection) {
    println!("SpecSlice select-tests · {} → {}", report.base, report.head);
    println!(
        "改动文件 {} · 改动符号 {} · 推荐测试 {}",
    ...
    if report.tests.is_empty() {
        ...
        println!(
            "提示: 这不意味着完全无风险。请先用 `specslice impact --base {base}` 查看业务侧影响，再决定是否手动补跑端到端测试。",
            base = report.base
        );
        return;
    }
