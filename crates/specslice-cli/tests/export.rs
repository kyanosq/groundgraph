//! CLI e2e tests for `specslice export --format jsonl`.

use assert_cmd::Command;
use assert_fs::prelude::*;
use predicates::str::contains;

/// MVP-0 acceptance: even on an empty graph, `specslice export --format
...
fn export_jsonl_emits_valid_files_for_empty_graph() {
    ...
    Command::cargo_bin("specslice")
        .expect("locate specslice binary")
    ...
    Command::cargo_bin("specslice")
        ...
        .assert()
        .success()
        .stdout(contains(".specslice/export"));

    let export_dir = repo.child(".specslice/export");
