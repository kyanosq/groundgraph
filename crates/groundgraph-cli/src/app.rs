...
enum Commands {
    ...
    /// Graph 快照比对 — 对比两份 `.groundgraph/graph.db` (P19)。
    ...
    /// 命令使用统计 (P26) — 汇总 `.groundgraph/stats.jsonl`：每命令调用次数 /
...
struct StatsArgs {
    ...
    #[arg(long)]
    json: bool,
    /// 清空统计账本（删除 `.groundgraph/stats.jsonl`）。
    #[arg(long)]
    reset: bool,
}
...
struct RouteCoverageArgs {
    ...
    #[arg(long, value_name = "N", default_value_t = groundgraph_engine::route_coverage::DEFAULT_SUFFIX_SEGMENTS)]
...
struct GraphDiffArgs {
    ...
    /// 会额外读取两边的 `.groundgraph/candidates/business_logic.yaml`
...
struct DashboardArgs {
    /// 输出文件路径（默认 `.groundgraph/export/dashboard.html`）。
...
struct SimilarArgs {
    ...
    #[arg(long, value_name = "N", default_value_t = groundgraph_engine::similarity::DEFAULT_MIN_TOKENS)]
    min_tokens: usize,
    ...
    #[arg(long, value_name = "K", default_value_t = groundgraph_engine::similarity::DEFAULT_SHINGLE_K)]
    shingle_k: usize,
    /// 进入 O(N²) 近似比对的最大符号数；超过该值跳过 near tier 并在 stats 标记。默认 20000。
    #[arg(long, value_name = "N", default_value_t = groundgraph_engine::similarity::DEFAULT_MAX_PAIRWISE_SYMBOLS)]
    max_pairwise: usize,
    /// 输出格式：`text`、`json`。
    #[arg(long, value_name = "FORMAT", default_value = "text")]
    format: TextJsonFormatArg,
...
impl DeadCodeConfidenceArg {
    fn into_engine(self) -> groundgraph_engine::dead_code::DeadCodeConfidence {
        use groundgraph_engine::dead_code::DeadCodeConfidence;
        match self {
...
struct GraphArgs {
    ...
    /// Mermaid, and to `.groundgraph/export/graph.html` for HTML.
    #[arg(long)]
    out: Option<PathBuf>,
    ...
    include_risks: bool,
    /// Overlay the `.groundgraph/candidates/business_logic.yaml` candidates
...
impl From<GraphViewArg> for groundgraph_engine::graph::GraphView {
    fn from(value: GraphViewArg) -> Self {
        match value {
            GraphViewArg::Overview => groundgraph_engine::graph::GraphView::Overview,
            GraphViewArg::Code => groundgraph_engine::graph::GraphView::Code,
            GraphViewArg::Business => groundgraph_engine::graph::GraphView::Business,
            GraphViewArg::Focus => groundgraph_engine::graph::GraphView::Focus,
        }
    }
}
...
enum CandidateSub {
    ...
    /// 写回审阅结果到 `.groundgraph/candidates/business_logic.yaml`。
...
struct ProposeArgs {
    ...
    /// 写入文件而非 stdout（如 `.groundgraph/export/business-pack.md`）。
    #[arg(long)]
    out: Option<PathBuf>,
    /// `--format json` 时美化输出。
    #[arg(long)]
...
struct BusinessDocArgs {
    ...
    /// 写入文件而非 stdout（如 `.groundgraph/export/business-doc.md`）。
    #[arg(long)]
    out: Option<PathBuf>,
...
struct SearchArgs {
    ...
    /// `<repo_root>/.groundgraph/export/search-<slug>.html`；当
    /// `--format mermaid` 时直接写入指定路径（默认打印到 stdout）。
    /// 显式传入的路径不受 `.groundgraph/` 非侵入约束——这是用户主动
    /// 指定的输出位置；省略该参数时 GroundGraph 永远只写 `.groundgraph/`。
...
struct ConnectApplyArgs {
    ...
    #[arg(long)]
    candidates: PathBuf,
    /// Validate only — do not modify `.groundgraph/links.yaml`.
    #[arg(long)]
    dry_run: bool,
...
impl From<ExportFormatArg> for groundgraph_engine::ExportFormat {
    fn from(value: ExportFormatArg) -> Self {
        match value {
            ExportFormatArg::Jsonl => groundgraph_engine::ExportFormat::Jsonl,
        }
    }
}
        ...
        Commands::Candidate(args) => match args.sub {
            ...
            CandidateSub::Review(a) => {
                let status = if a.accept {
                    groundgraph_engine::ReviewStatus::Accepted
                } else if a.reject {
                    groundgraph_engine::ReviewStatus::Rejected
                } else if a.needs_changes {
                    groundgraph_engine::ReviewStatus::NeedsChanges
                } else if a.pending {
                    groundgraph_engine::ReviewStatus::Pending
                } else {
                    anyhow::bail!(
...
mod tests {
    ...
    fn json_flag_conflicts_with_explicit_format() {
        for argv in [
            vec!["groundgraph", "search", "foo", "--format", "html", "--json"],
            vec!["groundgraph", "impact", "--format", "mermaid", "--json"],
            vec![
                "groundgraph",
                "candidate",
                "show",
    ...
    fn json_flag_alone_and_format_alone_still_parse() {
        assert!(Cli::try_parse_from(["groundgraph", "search", "foo", "--json"]).is_ok());
        assert!(Cli::try_parse_from(["groundgraph", "search", "foo", "--format", "html"]).is_ok());
        assert!(Cli::try_parse_from(["groundgraph", "impact", "--json"]).is_ok());
        assert!(Cli::try_parse_from(["groundgraph", "candidate", "show", "x", "--json"]).is_ok());
    }
    ...
    fn empty_positional_arguments_are_rejected() {
        for argv in [
            vec!["groundgraph", "trace", ""],
            vec!["groundgraph", "slice", ""],
            vec!["groundgraph", "context", ""],
            vec!["groundgraph", "candidate", "show", ""],
            vec!["groundgraph", "candidate", "review", "", "--accept"],
            vec!["groundgraph", "trace", "   "],
        ] {
            assert!(
    ...
    fn out_flag_is_unified_with_output_alias() {
        assert!(Cli::try_parse_from(["groundgraph", "graph", "--out", "g.json"]).is_ok());
        for flag in ["--out", "--output"] {
            assert!(
                Cli::try_parse_from(["groundgraph", "search", "q", flag, "s.html"]).is_ok(),
                "search must accept {flag}"
            );
            assert!(
                Cli::try_parse_from(["groundgraph", "impact", flag, "i.json"]).is_ok(),
    ...
    fn select_tests_and_impact_share_default_base_ref() {
        let select_base = match Cli::try_parse_from(["groundgraph", "select-tests"])
            .unwrap()
            .command
        {
        ...
        let impact_base = match Cli::try_parse_from(["groundgraph", "impact"])
            .unwrap()
            .command
        {
    ...
    fn select_tests_accepts_worktree_like_impact() {
        assert!(
            Cli::try_parse_from(["groundgraph", "select-tests", "--base", "HEAD", "--worktree"])
                .is_ok(),
            "select-tests must accept --worktree for parity with impact"
        );
        let worktree = match Cli::try_parse_from(["groundgraph", "select-tests", "--worktree"])
            .unwrap()
            .command
        {
