//! `groundgraph dead-code` — reports symbols that no entry point can
//! reach, with explicit confidence + reasons. Never auto-deletes.
//!
//! ```text
//! groundgraph dead-code                       # default: medium+
//! groundgraph dead-code --json                # JSON for agents
//! groundgraph dead-code --min-confidence high # only confident hits
//! groundgraph dead-code --include-tests       # also report orphan tests
//! ```
...
fn print_human(report: &DeadCodeReport) {
    ...
    println!();
    println!(
        "提示: 该报告不会自动删除任何文件。先用 `groundgraph graph --focus <id>` 或 `groundgraph search` 确认符号没有被反射 / 代码生成 / 外部消费者使用，再做删除决策。"
    );
    let block = render_warnings_block(&report.warnings);
    if !block.is_empty() {
        print!("{block}");
    }
}

/// Build the human-readable warnings tail (`== Warnings ==`). Returns
/// an empty string when there is nothing to render so callers can
/// guard `print!` without printing a stray blank line.
...
mod tests {
    ...
    fn render_warnings_block_lists_each_warning_with_header_and_dash() {
        let block = render_warnings_block(&[
            "warn: 节点 X 的入边质量查询失败：sqlite locked".to_string(),
            "warn: 节点 Y 的入边质量查询失败：io error".to_string(),
        ]);
        assert!(
