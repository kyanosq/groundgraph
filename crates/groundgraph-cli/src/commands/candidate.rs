...
//!
//! 三条入口：
//! - `groundgraph candidate list`     列出待审 / 已审候选（中文输出，带编号）
//! - `groundgraph candidate show <id>` 查看单个候选的完整业务描述、证据、风险、待确认问题
//! - `groundgraph candidate review <id> --accept|--reject|--needs-changes|--pending [--note "..."] [--answer "Q…"]`
//!   将审阅结果写回 `.groundgraph/candidates/business_logic.yaml`。
...
/// P14 — output format for `groundgraph candidate show`.
...
pub fn run_show(repo_root: &Path, candidate_id: &str, format: CandidateShowFormat) -> Result<i32> {
    ...
    let Some(c) = candidate else {
        eprintln!("groundgraph: 找不到候选 `{candidate_id}`。");
        return Ok(2);
    };
    match format {
...
pub fn render_candidate_mermaid(c: &BusinessCandidate) -> String {
    ...
    let notes = vec![format!(
        "groundgraph candidate show {} status={} evidence={} confidence={}",
        c.id,
        status,
        c.evidence.len(),
        c.confidence
...
fn print_human_list(snapshot: &CandidateListSnapshot, mode: ListMode) {
    ...
    println!();
    println!(
        "提示: `groundgraph candidate show <id>` 查看详情；\
         `groundgraph candidate review <id> --accept|--reject|--needs-changes|--pending --note \"...\"` 写回审阅结果。"
    );
}
...
fn print_human_show(c: &BusinessCandidate) {
    ...
    println!();
    println!(
        "动作: `groundgraph candidate review {} --accept|--reject|--needs-changes|--pending --note \"...\"`",
        c.id
    );
}
...
fn print_json(snapshot: &CandidateListSnapshot, mode: ListMode) -> Result<()> {
    ...
    struct CandidateSummary<'a> {
        ...
        review: Option<&'a groundgraph_engine::CandidateReview>,
    }
    #[derive(Serialize)]
    struct Out<'a> {
...
mod mermaid_tests {
    use super::*;
    use groundgraph_engine::{BusinessCandidate, CandidateReview};
