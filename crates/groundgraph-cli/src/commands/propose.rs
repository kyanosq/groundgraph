//! `groundgraph propose` — generate a per-business-module evidence pack
...
use groundgraph_engine::business_pack::{
...
fn render_markdown(pack: &BusinessPack) -> String {
    ...
    s.push_str("# 业务模块证据包（GroundGraph propose）\n\n");
    s.push_str("> 由 `groundgraph propose` 基于代码/文档/测试事实自动生成（非侵入、只读图）。\n");
    s.push_str("> 以下为供 AI 提炼业务逻辑的**证据**，不是已确认 requirement；确认请走 `groundgraph candidate review`。\n\n");
    s.push_str(&format!(
    ...
    s.push_str("把本文件（或 `--format json` 的证据包）连同下面的提示词交给你信任的模型，让它产出 `.groundgraph/candidates/business_logic.yaml`：\n\n");
...
fn render_text(pack: &BusinessPack) -> String {
    let mut s = String::new();
    s.push_str("GroundGraph propose · 业务模块证据包\n");
    s.push_str(&format!(
        "模块 {}/{} · 符号 {} (已归属 {}) · 文档 {} · 测试 {}\n\n",
        pack.stats.modules_reported,
        pack.stats.total_modules,
    ...
    if pack.modules.is_empty() {
        s.push_str("(图太稀疏 — 先 `groundgraph index`，或确认 code roots 配置)\n");
        return s;
    }
    for m in &pack.modules {
        s.push_str(&format!(
    ...
    s.push_str("\n下一步: `groundgraph propose --format markdown --out .groundgraph/export/business-pack.md` 再喂给 AI 生成 business_logic.yaml。\n");
    s
}
...
mod tests {
    ...
    fn sample_pack() -> BusinessPack {
        BusinessPack {
            ...
            prompt: "你是 GroundGraph 的业务逻辑提炼器。输出 business_logic.yaml。".into(),
        }
    }
