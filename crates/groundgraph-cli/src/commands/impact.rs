...
use groundgraph_engine::slice::SliceItem;
use groundgraph_engine::{run_impact, ImpactOptions, ImpactReport};

use crate::commands::graph_mermaid::{render_parts, MermaidEdge, MermaidNode};

/// P14 — output format for `groundgraph impact`.
...
pub fn render_impact_mermaid(report: &ImpactReport) -> String {
    ...
    let notes = vec![format!(
        "groundgraph impact — changed_files={} changed_symbols={} affected_requirements={} \
...
fn render_human(report: &ImpactReport) -> String {
    ...
    let _ = writeln!(out, "GroundGraph Impact Report");
    let _ = writeln!(out);
...
mod tests {
    ...
    fn sample_report() -> ImpactReport {
        use groundgraph_engine::impact::ImpactEdge;
        ImpactReport {
    ...
    fn impact_mermaid_renders_changed_files_requirements_tests_and_candidates() {
        ...
        assert!(
            out.contains("groundgraph impact"),
