...
pub fn run(repo_root: &Path, docs_only: bool) -> Result<()> {
    ...
    if !docs_only {
        match groundgraph_engine::unindexed_present_languages(repo_root) {
    ...
    if !docs_only {
        match groundgraph_engine::schema_indexer::index_schema(repo_root) {
            Ok(s) => {
                ...
                groundgraph_engine::stats::set_metric("http_routes", s.http_routes as i64);
                groundgraph_engine::stats::set_metric(
                    "route_method_edges",
                    s.route_method_edges as i64,
                );
                groundgraph_engine::stats::set_metric("consumed_routes", s.consumed_routes as i64);
                groundgraph_engine::stats::set_metric(
                    "route_consumer_edges",
                    s.route_consumer_edges as i64,
                );
                groundgraph_engine::stats::set_metric("tables", (s.sql_tables + s.orm_tables) as i64);
                groundgraph_engine::stats::set_metric(
                    "implicit_orm_tables",
                    s.implicit_orm_tables as i64,
                );
                groundgraph_engine::stats::set_metric("external_tables", s.external_tables as i64);
                groundgraph_engine::stats::set_metric("columns", s.columns as i64);
                groundgraph_engine::stats::set_metric("mapper_stmts", s.mapper_stmts as i64);
                groundgraph_engine::stats::set_metric(
                    "data_layer_edges",
                    (s.stmt_method_edges + s.stmt_table_edges) as i64,
                );
                groundgraph_engine::stats::set_metric("iface_impl_edges", s.iface_impl_edges as i64);
                groundgraph_engine::stats::set_metric(
...
/// `groundgraph stats` can report how much each `index` run produced.
fn record_index_metrics(result: &IndexResult) {
    use groundgraph_engine::stats::set_metric;
    let mut files = 0i64;
...
/// Render the human-readable summary for `groundgraph index`. Factored
...
pub(crate) fn format_result(result: &IndexResult) -> String {
    ...
    if let Some(ts) = &result.typescript {
        ...
        if ts.parse_timeouts > 0 {
            writeln!(
                out,
                "  warn: {} 个文件解析超出预算被跳过（多为含故意语法错误的 fixture；可用 GROUNDGRAPH_PARSE_BUDGET_MS 调整）",
                ts.parse_timeouts
            )
            .ok();
        }
    ...
    if !result.treesitter.is_empty() {
        ...
        for lang in &result.treesitter {
            ...
            if lang.parse_timeouts > 0 {
                writeln!(
                    out,
                    "  warn: {} 个 {} 文件解析超出预算被跳过（多为含故意语法错误的 fixture；可用 GROUNDGRAPH_PARSE_BUDGET_MS 调整）",
    ...
    {
        use groundgraph_engine::scip_runner::ScipRunStatus;
...
mod tests {
    ...
    fn render_includes_rust_section_with_treesitter_resolver_when_indexed() {
        let result = IndexResult {
            rust: Some(groundgraph_engine::RustIndexResult {
                files: 12,
                symbols: 140,
                imports: 60,
                references: 85,
    ...
    fn render_includes_scip_indexers_section_for_generated_and_skipped() {
        use groundgraph_engine::scip_runner::{ScipRunOutcome, ScipRunStatus};
        let result = IndexResult {
            scip_runs: vec![
                ScipRunOutcome {
                    language: "rust".into(),
                    status: ScipRunStatus::Generated,
                    output: Some(std::path::PathBuf::from(".groundgraph/scip/rust.scip")),
                },
                ScipRunOutcome {
                    language: "go".into(),
    ...
    fn render_includes_scip_section_when_overlay_ingested_files() {
        let result = IndexResult {
            scip: Some(groundgraph_engine::scip_overlay::ScipOverlayResult {
                scip_files: 2,
                documents: 318,
                edges: 9681,
                suppressed: 142,
    ...
    fn render_omits_scip_section_when_no_scip_file_on_disk() {
        ...
        let result = IndexResult {
            scip: Some(groundgraph_engine::scip_overlay::ScipOverlayResult::default()),
            ..IndexResult::default()
        };
        let out = format_result(&result);
        assert!(
    ...
    fn render_includes_unified_treesitter_section_per_language() {
        let result = IndexResult {
            treesitter: vec![
                groundgraph_engine::TreeSitterLangResult {
