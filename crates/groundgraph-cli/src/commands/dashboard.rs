//! `groundgraph dashboard` — one-shot self-contained HTML management panel.
//!
...
use groundgraph_engine::business_pack::{propose_business_pack, BusinessPackOptions};
use groundgraph_engine::checks::{run_checks, CheckOptions};
use groundgraph_engine::config::{storage_path_for_repo, DEFAULT_STORAGE_DIR};
use groundgraph_engine::dead_code::{analyze_dead_code, DeadCodeOptions};
use groundgraph_engine::feature_map::{analyze_feature_map, FeatureMapOptions};
use groundgraph_engine::questions::{analyze_questions, QuestionsOptions};
use groundgraph_engine::symbol_facts::{analyze_symbol_facts, Purity, SymbolFactsOptions};
...
fn collect(repo_root: &Path) -> Value {
    ...
    json!({
        "meta": {
            ...
            "tool": format!("groundgraph {}", env!("CARGO_PKG_VERSION")),
        },
        "overview": overview,
        "modules": modules,
        "features": features,
...
fn overview(repo_root: &Path) -> Result<Value> {
    let db = storage_path_for_repo(repo_root)?;
    let store =
...
<title>GroundGraph Dashboard</title>
<style>
...
(function(){
  ...
  html+='<p class="hint">交互式代码图请用 <code>groundgraph graph --format web</code> 生成。</p>';
  s.innerHTML=html;
})();
...
mod tests {
    ...
    fn collect_embeds_repo_name_not_host_absolute_path() {
        ...
        std::fs::create_dir_all(repo.join(".groundgraph")).unwrap();
