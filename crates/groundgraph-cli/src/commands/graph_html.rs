//! Self-contained HTML renderer for `groundgraph graph --format html`.
//!
...
pub fn render_html(view: &GraphViewModel) -> String {
    ...
    out.push_str("<script id=\"groundgraph-data\" type=\"application/json\">");
...
const RENDERER_JS: &str = r#"(function () {
  ...
  const dataEl = document.getElementById('groundgraph-data');
  let view;
  ...
  function computeEmptyMessage() {
    if (currentView === 'business') {
      ...
      return 'No confirmed business logic yet. Run `groundgraph connect propose`.';
    }
  ...
  document.getElementById('view').addEventListener('change', (ev) => {
    ...
    const detail = document.getElementById('detail');
    detail.innerHTML = '<h2>View</h2><p class="placeholder">Rerun <code>groundgraph graph --view ' +
...
mod tests {
    ...
    fn render_html_embeds_explorer_layout() {
        ...
        assert!(html.contains("<script id=\"groundgraph-data\""));
