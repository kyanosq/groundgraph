//! `groundgraph suggest-tests` — test suggestions from facts (P24 gap #5).
//!
//! ```text
//! groundgraph suggest-tests                 # prioritised checklist, branchy first
//! groundgraph suggest-tests --only-pure     # cheap deterministic wins first
//! groundgraph suggest-tests --max 20        # top 20 highest-value symbols
//! groundgraph suggest-tests --json          # machine-readable for an agent
//! ```

use std::path::PathBuf;

use anyhow::{Context, Result};
use groundgraph_engine::test_suggestions::{
...
fn print_human(report: &TestSuggestionsReport) {
    println!("GroundGraph 测试建议 (schema v{})", report.schema_version);
    println!(
        "分析 {} · 有建议 {} · 建议总数 {} · 输出 {}{}",
