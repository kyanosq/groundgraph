//! `groundgraph business-doc` — render the human-confirmed business
...
use groundgraph_engine::business_doc::{
...
fn render_markdown(doc: &BusinessDoc) -> String {
    ...
    s.push_str("# 业务逻辑文档（GroundGraph）\n\n");
    s.push_str("> 由 `groundgraph business-doc` 基于**已确认**的业务候选（`.groundgraph/candidates/business_logic.yaml`）生成。\n");
    s.push_str(
    ...
    if doc.entries.is_empty() {
        ...
        s.push_str("1. `groundgraph propose --format markdown --out .groundgraph/export/business-pack.md` 生成证据包；\n");
        s.push_str("2. 交给 AI 产出 `.groundgraph/candidates/business_logic.yaml`；\n");
        s.push_str("3. `groundgraph candidate review` 确认；\n");
...
fn render_text(doc: &BusinessDoc) -> String {
    ...
    if doc.entries.is_empty() {
        s.push_str(
            "（无已确认业务能力；先 `groundgraph candidate review`，或加 --include-proposed 预览）\n",
        );
        return s;
    }
    for e in &doc.entries {
