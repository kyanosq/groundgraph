...
//! `groundgraph search --format html`.
//!
...
use anyhow::Result;
use groundgraph_engine::search::SearchHtmlPayload;
...
pub fn render_html(payload: &SearchHtmlPayload) -> Result<String> {
    ...
    out.push_str("<script id=\"groundgraph-search-data\" type=\"application/json\">");
    out.push_str(&safe);
    out.push_str("</script>\n");
    out.push_str("<script>\n");
...
<head>
  ...
  <title>GroundGraph Search Reader</title>
  <style>
    :root {
      --bg: #f7f7f4;
      --panel: #ffffff;
...
(function () {
  const dataEl = document.getElementById('groundgraph-search-data');
  if (!dataEl) return;
  let payload;
