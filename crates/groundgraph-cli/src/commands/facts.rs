//! `groundgraph facts` — behavioural fact extraction (P24 gap #1) and
//! `groundgraph purity` — node purity classification (P24 gap #6).
//!
//! ```text
//! groundgraph facts                      # branches/returns/compares + evidence lines
//! groundgraph facts --purity pure        # only pure callables (great seeds for tests)
//! groundgraph facts --json               # machine-readable for an agent
//! groundgraph purity                     # pure / impure / unknown census + reasons
//! ```
//!
...
use groundgraph_engine::symbol_facts::{
...
fn print_facts_human(report: &SymbolFactsReport) {
    println!("GroundGraph 行为事实 (schema v{})", report.schema_version);
    println!(
        "分析 {} · 有源码 {} · 纯 {} · 有副作用 {} · 未知 {} · 输出 {}{}",
...
fn print_purity_human(report: &SymbolFactsReport) {
    println!("GroundGraph 纯度普查 (schema v{})", report.schema_version);
    println!(
        "分析 {} · 纯 {} · 有副作用 {} · 未知 {}",
