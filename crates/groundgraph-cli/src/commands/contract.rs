//! `groundgraph contract` — data contract view (P24 gap #3).
//!
//! ```text
//! groundgraph contract               # SQL CREATE TABLE schemas + JSON keymaps
//! groundgraph contract --tables-only # only persistence schema
//! groundgraph contract --keys-only   # only serialization keymap
//! groundgraph contract --json        # machine-readable for an agent
//! ```

use std::path::PathBuf;

use anyhow::{Context, Result};
use groundgraph_engine::data_contract::{
...
fn print_human(report: &DataContractReport) {
    println!("GroundGraph 数据契约 (schema v{})", report.schema_version);
    println!(
