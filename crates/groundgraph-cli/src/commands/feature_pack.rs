//! `groundgraph feature-pack` — one-click feature slice export (P24 gap #7).
//!
...
//!
//! ```text
//! groundgraph feature-pack --path lib/alarm           # everything under a dir
//! groundgraph feature-pack --requirement REQ-ALARM    # files a requirement touches
//! groundgraph feature-pack --path lib/alarm --format text   # human summary
//! ```
...
fn print_human(pack: &FeaturePack) {
    println!("GroundGraph 特性切片包 (schema v{})", pack.schema_version);
...
fn purity_str(p: groundgraph_engine::symbol_facts::Purity) -> &'static str {
    use groundgraph_engine::symbol_facts::Purity;
    match p {
        Purity::Pure => "pure",
