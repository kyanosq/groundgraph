//! CLI e2e tests for `groundgraph stats` (P26).
//!
//! Every invocation appends one record to `.groundgraph/stats.jsonl`; `stats`
...
fn stats_reset_reports_when_ledger_absent() {
    ...
    let tmp = tempfile::TempDir::new().unwrap();
    bootstrap(tmp.path());
    let ledger = tmp.path().join(".groundgraph").join("stats.jsonl");
    if ledger.exists() {
