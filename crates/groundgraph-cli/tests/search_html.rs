//! P6 — `groundgraph search --format html` CLI integration test.
//!
...
fn search_html_writes_self_contained_reader_to_default_path() {
    ...
    let html_path = tmp.path().join(".groundgraph/export/search-watermark.html");
    assert!(
        html_path.exists(),
    ...
    assert!(html.contains("GroundGraph Search"));
    assert!(html.contains("groundgraph-search-data"));
...
fn search_text_format_default_keeps_human_output_compatible() {
    ...
    assert!(stdout.contains("GroundGraph search"));
