...
use groundgraph_store::{FileIndexEntry, FulltextRow, Store};
...
fn evidence_can_round_trip_every_kind() {
    let (_tmp, mut store) = fresh_store();
    use groundgraph_core::EvidenceKind;
    for (idx, kind) in [
...
fn edges_can_round_trip_every_kind_and_source_and_status() {
    ...
    for (idx, kind) in kinds.iter().enumerate() {
        ...
        if idx % 2 == 0 {
            edge.status = groundgraph_core::EdgeStatus::Deprecated;
        }
...
fn repeated_fulltext_rebuilds_stay_queryable_after_optimize() {
    ...
    // `'optimize'` merge. Simulate many `groundgraph index` runs and assert the
