...
//! - the user has explicitly enabled it via `GROUNDGRAPH_DART_ANALYZER=1`,
//!   or
//! - the user pointed `GROUNDGRAPH_DART_ANALYZER_BIN` at a built binary
//!   (or the default `dart run tool/groundgraph_dart_analyzer/...`
...
//! ([`groundgraph_lang_dart::index_dart_paths`]) silently, preserving the
...
use groundgraph_core::language_batch::LanguageIndexBatch;
...
pub const ENV_ENABLE: &str = "GROUNDGRAPH_DART_ANALYZER";

/// Environment variable used to override the sidecar entry-point.
///
/// Accepts either an absolute path to a compiled `groundgraph_dart_analyzer`
/// binary or a shell-style command (split on whitespace). If unset, the
/// engine defaults to `dart run tool/groundgraph_dart_analyzer/bin/groundgraph_dart_analyzer.dart`
/// relative to the repo root.
pub const ENV_BIN: &str = "GROUNDGRAPH_DART_ANALYZER_BIN";

/// Override the sidecar wall-clock budget (seconds, default 600). A hung
/// analyzer otherwise blocked `groundgraph index` forever (issues2.md #48).
pub const ENV_TIMEOUT_SECS: &str = "GROUNDGRAPH_DART_ANALYZER_TIMEOUT_SECS";

/// Default workspace location of the sidecar entry point. Kept in sync
/// with the file we ship at `tool/groundgraph_dart_analyzer/`.
...
fn is_enabled() -> bool {
    ...
    // ladder is:
    // - `GROUNDGRAPH_DART_ANALYZER=0` / `false` / `off` / `no` -> disabled.
    // - `GROUNDGRAPH_DART_ANALYZER=1` / `true` / `yes` / `on`  -> enabled.
    // - unset                                                   -> enabled
    ...
    let raw = std::env::var(ENV_ENABLE);
    match raw {
...
fn probe_locations(repo_root: &Path) -> Vec<PathBuf> {
    ...
    // Co-located alongside the `groundgraph` binary — works for a
    // pre-packaged install where the sidecar source is shipped next to
    // the executable.
    if let Ok(exe) = std::env::current_exe() {
        if let Some(parent) = exe.parent() {
            out.push(parent.join(DEFAULT_SIDECAR_REL));
            out.push(parent.join("groundgraph_dart_analyzer.dart"));
            // …/target/<profile>/groundgraph → walk back to the workspace
...
fn resolve_command_with(_repo_root: &Path, locations: &[PathBuf]) -> Option<Command> {
    ...
    for candidate in locations {
        if candidate.exists() {
            return Some(command_for_sidecar_script(candidate));
        }
    }
    None
}
...
fn command_from_str(raw: &str) -> Command {
    ...
    if parts.len() >= 3 && parts[0] == "dart" && parts[1] == "run" {
        let script = PathBuf::from(&parts[2]);
        if sidecar_package_root_for_script(&script).is_some() {
            let mut cmd = command_for_sidecar_script(&script);
...
fn command_for_sidecar_script(script: &Path) -> Command {
    let mut cmd = Command::new("dart");
    if let Some(package_root) = sidecar_package_root_for_script(script) {
        cmd.current_dir(package_root);
        cmd.arg("run")
            .arg(Path::new("bin").join(script.file_name().expect("sidecar script file name")));
    } else {
...
fn sidecar_package_root_for_script(script: &Path) -> Option<PathBuf> {
    let file = script.file_name()?.to_str()?;
    if file != "groundgraph_dart_analyzer.dart" {
        return None;
    }
    let bin_dir = script.parent()?;
...
fn write_request(
    ...
    let roots_owned: Vec<String> = code_roots
        ...
        .collect();
    let repo_root_abs = absolute_repo_root(repo_root)?;
...
fn absolute_repo_root(repo_root: &Path) -> Result<PathBuf> {
    ...
    Ok(std::env::current_dir()
        .context("resolve current directory for sidecar request")?
        .join(repo_root))
}
...
fn parse_response(stdout: &[u8]) -> SidecarOutcome {
    ...
    for (family, count) in dropped {
        batch
            .diagnostics
            .push(groundgraph_core::language_batch::AdapterDiagnostic {
                path: String::new(),
...
mod tests {
    ...
    #[test]
    fn dart_run_sidecar_file_uses_sidecar_package_root_as_cwd() {
        let script =
            Path::new("/tmp/tool/groundgraph_dart_analyzer/bin/groundgraph_dart_analyzer.dart");
        let cmd = command_from_str(&format!("dart run {}", script.display()));
        assert_eq!(
            cmd.get_current_dir(),
            Some(Path::new("/tmp/tool/groundgraph_dart_analyzer"))
        );
        assert_eq!(
            cmd.get_args()
                .map(|arg| arg.to_string_lossy().into_owned())
                .collect::<Vec<_>>(),
            vec!["run", "bin/groundgraph_dart_analyzer.dart"]
        );
    }
    ...
    fn skip_reason_lists_every_probed_path() {
        ...
        // `tool/groundgraph_dart_analyzer/...` at the repo root, so the
    ...
    fn probe_locations_includes_repo_root_and_binary_neighbour_and_home() {
        ...
        // `groundgraph` binary OR under a user-scoped analyzer directory
        ...
        assert!(
            joined.contains(".groundgraph/dart_analyzer"),
            "must probe ~/.groundgraph/dart_analyzer:\n{joined}"
        );
        assert!(
            !joined.contains(".specslice/dart_analyzer"),
            "must not probe ~/.specslice/dart_analyzer:\n{joined}"
        );
    ...
    #[test]
    fn sidecar_default_path_uses_groundgraph_name_only() {
        assert_eq!(
            DEFAULT_SIDECAR_REL,
            "tool/groundgraph_dart_analyzer/bin/groundgraph_dart_analyzer.dart"
        );
        ...
        let joined = probes
            ...
            .join("\n");
        assert!(
            joined.contains("groundgraph_dart_analyzer.dart"),
            "{joined}"
        );
        assert!(!joined.contains("specslice_dart_analyzer.dart"), "{joined}");
    }

    #[test]
    fn old_specslice_sidecar_env_vars_are_ignored() {
        with_env_lock(|| {
            with_env_var(ENV_ENABLE, None, || {
                with_env_var("SPECSLICE_DART_ANALYZER", Some("0"), || {
                    assert!(is_enabled());
                });
            });
            with_env_var(ENV_BIN, None, || {
                with_env_var(
                    ...
                    Some("dart run old.dart"),
                    || {
                        assert!(resolve_command_with(Path::new("."), &[]).is_none());
                    },
                );
            });
        });
    ...
    #[test]
    fn write_request_absolutizes_relative_repo_root() {
        let body = write_request(Path::new("."), &[PathBuf::from("lib")], &[]).unwrap();
        let v: serde_json::Value = serde_json::from_str(&body).unwrap();
        let repo_root = v["repo_root"].as_str().unwrap();
        assert!(
            Path::new(repo_root).is_absolute(),
            "sidecar repo_root must be absolute when its cwd is the sidecar package: {body}"
        );
        assert_eq!(v["code_roots"], serde_json::json!(["lib"]));
    }
