...
//!   heuristic [`groundgraph_lang_dart::index_dart_paths`] fallback — only
...
use groundgraph_core::language_batch::{FileArtifact, ImportEdge, LanguageIndexBatch};
use groundgraph_core::{EdgeAssertion, EdgeKind, EdgeSource, Node, NodeKind};
use groundgraph_lang_dart::index_dart_paths;
...
pub struct DartIndexOptions {
    ...
    /// behaviour; the `GROUNDGRAPH_DART_ANALYZER=0` env override is still
...
pub fn index_dart(store: &mut Store, options: &DartIndexOptions) -> Result<DartIndexResult> {
    ...
    // (`options.disable_analyzer`) or `GROUNDGRAPH_DART_ANALYZER=0`; it also
    ...
    for sym in &overlay.symbols {
        if sym.kind == NodeKind::DartProvider {
            batch.symbol_ranges.push(groundgraph_core::SymbolRange {
...
fn backfill_referenced_symbols(
    ...
    overlay_symbols: &[groundgraph_core::language_batch::SymbolArtifact],
) {
    ...
    let mut overlay_by_id: std::collections::HashMap<
        String,
        &groundgraph_core::language_batch::SymbolArtifact,
...
fn reconcile_misparsed_callables(
    ...
    let mut substitutes: Vec<groundgraph_core::language_batch::SymbolArtifact> = Vec::new();
    ...
    for s in &batch.symbols {
        ...
        let replacements: Vec<&groundgraph_core::language_batch::SymbolArtifact> = overlay_symbols
            .iter()
            .filter(|a| {
    ...
    let mut parent_adds: Vec<groundgraph_core::language_batch::SymbolArtifact> = Vec::new();
...
fn ingest(
    ...
    for symbol in &batch.symbols {
        ...
        let mut contains = if let Some(parent) = &symbol.parent_symbol_id {
            ...
            EdgeAssertion::fact(
                groundgraph_core::artifact_id::file_id(&symbol.path),
                symbol.id.clone(),
                EdgeKind::Contains,
                EdgeSource::LanguageAdapter,
            )
    ...
    for test in &batch.tests {
        ...
        let parent_id = test
            .parent_symbol_id
            .clone()
            .unwrap_or_else(|| groundgraph_core::artifact_id::file_id(&test.path));
    ...
    for import in &batch.imports {
        let mut edge = EdgeAssertion::fact(
            ...
            groundgraph_core::artifact_id::file_id(&import.to_path),
            EdgeKind::Imports,
            EdgeSource::LanguageAdapter,
        );
    ...
    for symbol in &batch.symbols {
        store.upsert_symbol_range(&groundgraph_core::SymbolRange {
...
mod reconcile_tests {
    use super::*;
    use groundgraph_core::artifact_id::{dart_class_id, dart_function_id, dart_method_id};
    use groundgraph_core::language_batch::{SymbolArtifact, SymbolRange};
    use groundgraph_core::NodeKind;

    fn sym(
        id: groundgraph_core::ArtifactId,
        kind: NodeKind,
        path: &str,
        name: &str,
    ) -> SymbolArtifact {
    ...
    fn drops_phantom_fn_for_extension_member_with_synthetic_parent() {
        ...
        method.parent_symbol_id = Some(groundgraph_core::ArtifactId::new(
...
mod ingest_tests {
    ...
    fn ingest_skips_unknown_edge_kinds_defensively() {
        ...
        let mut store = groundgraph_store::Store::open(tmp.path().join("g.db")).unwrap();
    ...
    fn ingest_accepts_calls_and_references_normally() {
        use groundgraph_core::artifact_id::dart_class_id;
        let tmp = tempfile::TempDir::new().unwrap();
        let mut store = groundgraph_store::Store::open(tmp.path().join("g.db")).unwrap();
        store.migrate().unwrap();
        let mut batch = LanguageIndexBatch {
    ...
    fn ingest_normalises_symbol_without_parent_to_file_contains() {
        ...
        use groundgraph_core::{artifact_id::dart_function_id, SymbolArtifact};
        let tmp = tempfile::TempDir::new().unwrap();
        let mut store = groundgraph_store::Store::open(tmp.path().join("g.db")).unwrap();
        store.migrate().unwrap();
        let mut batch = LanguageIndexBatch {
        ...
        batch.symbols.push(SymbolArtifact {
            id: fn_id.clone(),
            kind: groundgraph_core::NodeKind::DartFunction,
            path: "x.dart".into(),
            name: "top".into(),
