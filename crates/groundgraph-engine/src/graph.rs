//! P6 / P6.1: read-only `GraphViewModel` for GroundGraph visualisation.
//!
...
//! | `confirmed` | External manifest links (`.groundgraph/links.yaml`)  |
//! | `candidate` | `.groundgraph/candidates/`                           |
...
pub struct GraphOptions {
    ...
    /// `.groundgraph/candidates/business_logic.yaml` are merged into the
    ...
    /// engine also filters defensively at view time.
...
pub fn build_graph_view(repo_root: &Path, options: GraphOptions) -> EngineResult<GraphViewModel> {
    ...
    // `.groundgraph/candidates/business_logic.yaml`. Read-only: the engine
    ...
    if matches!(options.view, GraphView::Business) && should_emit_no_business_logic_finding(&nodes)
    {
        findings.push(GraphFinding {
            ...
            severity: "info".into(),
            message:
                "No confirmed business logic in graph. Run `groundgraph connect propose` to seed candidates."
                    .into(),
            target_id: None,
        });
    }
...
fn column_for(kind: NodeKind) -> GraphColumn {
    ...
    use groundgraph_core::language_traits::{family_of, SymbolFamily};
...
mod tests {
    ...
    fn layer_for_edge_promotes_external_manifest_only() {
        use groundgraph_core::{ArtifactId, EdgeKind};
    ...
    fn p63_map_edge_surfaces_evidence_fields_on_graph_edge() {
        use groundgraph_core::{ArtifactId, EdgeAssertion, EdgeKind, EdgeSource};
