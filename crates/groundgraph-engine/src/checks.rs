//! GroundGraph checks — declared-link consistency *and* content-level
//! doc→code drift.
//!
...
//!   implementations are suggested (`groundgraph connect` confirms them); zero
...
fn fulltext_mentions(store: &Store, identifier: &str) -> bool {
    ...
    hits.iter().any(|hit| {
        store
            .find_node(&groundgraph_core::ArtifactId::new(hit.node_id.clone()))
            .ok()
            .flatten()
            .is_some_and(|n| {
                !matches!(
...
fn classify_code_ref(span: &str) -> Option<CodeRef> {
    ...
    if s.starts_with(".groundgraph/") || s.starts_with(".groundgraph\\") {
        return None; // GroundGraph's own runtime workspace — docs cite outputs
    }
...
fn compute_requirement_hints(
    ...
    for req in store.list_nodes_by_kind(NodeKind::Requirement)? {
        ...
        let message = if candidates.is_empty() {
            ...
            format!(
                "需求 {} 未声明实现；图中疑似实现：{}（用 `groundgraph connect` 确认）",
                req.id,
                candidates.join("、")
            )
        };
