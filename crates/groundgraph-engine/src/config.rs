//! Workspace configuration stored in `.groundgraph.yaml`.
//!
//! MVP-0 only persisted `repo + storage`. The non-invasive MVP schema keeps
//! all GroundGraph-owned metadata under `.groundgraph/`; business docs and code
...
/// Schema version of the `.groundgraph.yaml` contract this build understands
/// (#72). Bump on any breaking change to a config key's name or semantics, and
/// add the matching migration / warning. Every other external GroundGraph
...
pub struct EnrichmentConfig {
    ...
    #[serde(default = "default_true")]
    pub analyzer: bool,
    /// Ingest offline SCIP indexes (`.groundgraph/scip/*.scip`) as a
...
impl EngineConfig {
    ...
    /// Forward-compat guard for `.groundgraph.yaml` (#72): a warning string when
...
pub fn config_schema_notice(declared: Option<u32>, supported: u32) -> Option<String> {
    match declared {
        Some(v) if v > supported => Some(format!(
            ".groundgraph.yaml declares schema_version {v}, but this groundgraph supports up to \
             {supported}; upgrade groundgraph or some config keys may be misinterpreted"
        )),
        _ => None,
    }
}
...
fn default_links_path() -> String {
    format!("{DEFAULT_STORAGE_DIR}/links.yaml")
}
...
/// Return the config file this repo should use.
pub fn config_path_for_repo(repo_root: &Path) -> PathBuf {
    repo_root.join(DEFAULT_CONFIG_FILE_NAME)
}

pub fn has_config_file(repo_root: &Path) -> bool {
...
/// Configuration consumed by `groundgraph dead-code` (P7).
///
...
pub struct LanguageAdapterConfig {
    ...
    /// `GROUNDGRAPH_SWIFT_LSP_BIN` env var takes precedence at runtime.
    #[serde(default)]
    pub lsp_command: Option<String>,
}
