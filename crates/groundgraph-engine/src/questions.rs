//! P19 — `groundgraph questions`: surface the unresolved facts an
...
//!    Imports edges that point at a node GroundGraph never created.
...
pub struct Question {
    ...
    /// fetch the surrounding context via `groundgraph graph
    /// --focus`.
...
pub fn analyze_questions_with_store(
    ...
    // `.groundgraph/candidates/business_logic.yaml` (graph.rs merges them
    ...
    // surface them.
    {
        ...
        // bubble up through `groundgraph candidate list` instead.
...
mod tests {
    ...
    /// `.groundgraph/candidates/business_logic.yaml` (`graph.rs` merges
