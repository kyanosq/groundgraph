...
//!    operator typing `groundgraph search "login auth session"`.
//!
...
//! `graph_commands` field embeds a `groundgraph graph --focus …`
...
use groundgraph_core::{ArtifactId, EdgeAssertion, Node, NodeKind};
...
pub struct SearchResult {
    ...
    /// operator — first entry is always a `groundgraph graph --focus …`
...
/// Bundle that drives `groundgraph search --format html`. Schema is
...
fn apply_content_layer(
    ...
    if !available {
        warnings.push(
            "warn: 当前索引缺少全文内容层（由旧版本生成）。重新运行 `groundgraph index` 后，\
...
fn build_graph_commands(matches: &[SearchMatch], repo_root: &Path) -> Vec<String> {
    ...
    if let Some(top) = matches.first() {
        out.push(format!(
            "groundgraph --repo-root {} graph --view focus --focus {} --format html",
...
mod tests {
    ...
    fn insert_method(store: &mut Store, file: &str, qualified: &str, line: (u32, u32)) -> String {
        ...
        store
            .upsert_symbol_range(&groundgraph_core::SymbolRange {
                file_path: file.into(),
    ...
    fn run_search_returns_subgraph_neighbours_at_depth_1() {
        ...
        std::fs::write(
            dir.path().join(".groundgraph.yaml"),
    ...
    fn code_search_prefers_direct_code_symbols_over_candidate_text_mentions() {
        let (mut store, dir) = empty_store();
        std::fs::create_dir_all(dir.path().join(".groundgraph/candidates")).unwrap();
        std::fs::write(
            dir.path()
