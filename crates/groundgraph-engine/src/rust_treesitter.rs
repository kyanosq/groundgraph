...
/// crate entry file (`lib.rs` / `main.rs`) or has binaries under `bin/*.rs`.
/// These anchor `crate::` resolution and the crate-name → directory mapping
/// used for cross-crate imports.
...
fn rust_src_roots(all_files: &[String]) -> Vec<String> {
    ...
    for file in all_files {
        ...
        if let Some(bin_dir) = file.rsplit_once("/bin/").map(|(dir, _)| dir) {
            if file.ends_with(".rs") && !bin_dir.is_empty() {
                roots.insert(bin_dir.to_string());
            }
        }
    }
...
/// Crate name derived from a src root (`crates/groundgraph-engine/src` →
/// `groundgraph_engine`), normalised the way `use` paths spell it.
...
"#;
    ...
    fn workspace_files() -> Vec<String> {
        [
            "crates/groundgraph-cli/src/app.rs",
            "crates/groundgraph-cli/src/bin/groundgraph.rs",
            "crates/groundgraph-cli/src/commands/mod.rs",
            "crates/groundgraph-cli/src/commands/search.rs",
            "crates/groundgraph-cli/src/commands/graph_mermaid.rs",
            "crates/groundgraph-engine/src/lib.rs",
            "crates/groundgraph-engine/src/graph.rs",
            "crates/groundgraph-engine/src/slice.rs",
        ]
        .into_iter()
        .map(String::from)
        .collect()
    ...
    fn src_roots_are_crate_entry_dirs_deepest_first() {
        ...
        assert!(
            roots.contains(&"crates/groundgraph-cli/src".to_string()),
            "{roots:?}"
        );
        assert!(
            roots.contains(&"crates/groundgraph-engine/src".to_string()),
            "{roots:?}"
        );
        // A non-entry directory must never be reported as a crate root.
        assert!(!roots.contains(&"crates/groundgraph-cli/src/commands".to_string()));
    }

    #[test]
    fn crate_name_strips_src_and_normalises_dashes() {
        assert_eq!(
            rust_crate_name("crates/groundgraph-engine/src").as_deref(),
            Some("groundgraph_engine")
        );
    }
    ...
    fn resolve_crate_relative_to_sibling_module() {
        ...
        assert_eq!(
            got.as_deref(),
            Some("crates/groundgraph-cli/src/commands/graph_mermaid.rs")
        );
    }
    ...
    fn resolve_super_glob_to_parent_module_file() {
        ...
        assert_eq!(
            got.as_deref(),
            Some("crates/groundgraph-cli/src/commands/mod.rs")
        );
    }
    ...
    fn resolve_cross_crate_import_by_crate_name() {
        ...
        let roots = rust_src_roots(&files);
        let got = rust_resolve_import(
            "groundgraph_engine::graph::GraphLayer",
            "crates/groundgraph-cli/src/commands/search.rs",
            &files,
            &roots,
        );
        assert_eq!(
            got.as_deref(),
            Some("crates/groundgraph-engine/src/graph.rs")
        );
    }
    ...
    fn cross_crate_reexport_falls_back_to_crate_entry() {
        ...
        let got = rust_resolve_import(
            "groundgraph_engine::GraphLayer",
            "crates/groundgraph-cli/src/commands/search.rs",
            &files,
            &roots,
        );
        assert_eq!(got.as_deref(), Some("crates/groundgraph-engine/src/lib.rs"));
    }
    ...
    fn self_import_is_not_emitted() {
        ...
        assert_eq!(
            got.as_deref(),
            Some("crates/groundgraph-cli/src/commands/search.rs")
        );
