//! P19 — `groundgraph graph-diff`: compare two GroundGraph graph
//! snapshots and report what changed.
//!
//! The MVP works on two `.groundgraph/graph.db` files (typically
...
//!   candidates live in `.groundgraph/candidates/business_logic.yaml`
...
pub struct GraphDiffOptions {
    ...
    /// repo's `.groundgraph/candidates/business_logic.yaml` and reports
...
pub struct DiffCandidate {
    ...
    /// use, so reviewers can paste it into `groundgraph candidate review`.
    pub id: String,
    pub name: String,
...
mod tests {
    use super::*;
    use groundgraph_core::{ArtifactId, EdgeKind, EdgeSource, EdgeStatus, NodeKind};
    ...
    fn write_candidates(dir: &TempDir, contents: &str) {
        let candidates_dir = dir.path().join(".groundgraph").join("candidates");
