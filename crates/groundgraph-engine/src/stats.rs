...
//! line to `<repo_root>/.groundgraph/stats.jsonl`. `groundgraph stats` then
...
/// File name (under `.groundgraph/`) of the append-only stats ledger.
...
/// read at `groundgraph stats` time (#250). The previous ledger is kept as a
...
/// Append one stat as a JSON line to `<groundgraph_dir>/stats.jsonl`.
///
/// Best-effort and non-fatal: if `groundgraph_dir` does not exist (command run
/// outside a workspace, e.g. `graph-equiv` over bare db paths) it does nothing,
/// so statistics never break a command.
pub fn append_stat(groundgraph_dir: &Path, stat: &CommandStat) -> std::io::Result<()> {
    append_stat_inner(groundgraph_dir, stat, MAX_STATS_BYTES)
}

/// [`append_stat`] with an injectable rotation cap (so tests can exercise
/// rotation without writing megabytes).
fn append_stat_inner(
    groundgraph_dir: &Path,
    stat: &CommandStat,
    max_bytes: u64,
) -> std::io::Result<()> {
    if !groundgraph_dir.is_dir() {
        return Ok(());
    }
    let path = groundgraph_dir.join(STATS_REL_PATH);
    ...
    if let Ok(meta) = std::fs::metadata(&path) {
        if meta.len() >= max_bytes {
            let rotated = groundgraph_dir.join(format!("{STATS_REL_PATH}.1"));
    ...
    // `write` syscalls (large metrics maps), and concurrent groundgraph
...
/// every record — the memory-safe path for `groundgraph stats` (#250).
