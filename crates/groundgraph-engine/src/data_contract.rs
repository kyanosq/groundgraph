...
mod tests {
    ...
    fn report_aggregates_keys_across_sites() {
        ...
        .unwrap();
        let mut store = groundgraph_store::Store::open(tmp.path().join("graph.db")).unwrap();
        store.migrate().unwrap();
        let mut n = groundgraph_core::Node::new(
            groundgraph_core::ArtifactId::new("dart::lib/model.dart#M"),
            groundgraph_core::NodeKind::DartClass,
        );
