...
/// `.groundgraph/` workspace would otherwise flood the graph with thousands of
...
fn is_pruned_dir(entry: &walkdir::DirEntry) -> bool {
    ...
    // A nested GroundGraph workspace (a sub-directory, depth > 0, holding its
    // own `.groundgraph.yaml`) is a separate project — vendored reference repos
    // carry their own config — indexed by its own `index`, never folded into
    // the parent doc graph. The doc root itself (depth 0) is exempt.
    if entry.depth() > 0 && crate::config::has_config_file(entry.path()) {
        return true;
    }
...
mod tests {
    ...
    /// `.groundgraph.yaml`; it is a separate workspace, indexed by its own
    ...
    #[test]
    fn docs_walk_prunes_nested_groundgraph_workspaces() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        let vendored = root.join("docs/references/vendored");
        std::fs::create_dir_all(&vendored).unwrap();
        std::fs::write(root.join("docs/guide.md"), "# Guide\n\nbody\n").unwrap();
        std::fs::write(vendored.join(".groundgraph.yaml"), "repo:\n  root: .\n").unwrap();
    ...
    #[test]
    fn release_scan_snapshots_do_not_become_docs() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        std::fs::create_dir_all(root.join("workspace-member")).unwrap();
        std::fs::create_dir_all(root.join("release-scans/_scratch/extrepo")).unwrap();
        std::fs::write(root.join("Cargo.toml"), "[workspace]\n").unwrap();
        std::fs::write(root.join("workspace-member/Cargo.toml"), "[package]\n").unwrap();
        std::fs::write(
            root.join("workspace-member/README.md"),
            "# Workspace member\n\nbody\n",
        )
        .unwrap();
        std::fs::write(
            root.join("release-scans/_scratch/extrepo/package.json"),
            "{}",
        )
        .unwrap();
        std::fs::write(
            root.join("release-scans/_scratch/extrepo/README.md"),
            "# External snapshot\n\nnoise\n",
        )
        .unwrap();
        ...
        assert!(
            nodes
                .iter()
                .any(|n| n.id.to_string() == "file::workspace-member/README.md"),
            "workspace package docs must still be indexed; got {result:?}"
        );
        assert!(
            nodes
                .iter()
                .all(|n| !n.id.to_string().contains("release-scans")),
            "release scan snapshots must stay pruned; got {result:?}"
        );
    }

    /// A vendored project copy inside the docs tree (groundgraph dogfood:
    ...
    fn asciidoc_headings_become_doc_sections() {
        ...
        let titles: Vec<&str> = nodes
            .iter()
            .filter(|n| n.kind == groundgraph_core::NodeKind::DocSection)
