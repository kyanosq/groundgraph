...
//!
//! GroundGraph stays non-invasive: requirement↔code/doc/test relationships live
//! under `.groundgraph/requirements/*.md`, never in the target source. This is
//! the recommended, human-friendly successor to `.groundgraph/links.yaml`
...
use groundgraph_core::artifact_id::{doc_section_id, file_id, requirement_id, slugify};
...
pub const DEFAULT_REQUIREMENTS_DIR: &str = ".groundgraph/requirements";
...
mod tests {
    ...
    fn cross_file_duplicate_requirement_ids_are_reported() {
        ...
        let root = tmp.path();
        let reqdir = root.join(".groundgraph/requirements");
