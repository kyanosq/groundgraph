...
//!
//! ```
//! GROUNDGRAPH_SWIFT_LSP_BIN=/path/to/sourcekit-lsp \
...
fn open_temp_store(repo: &Path) -> (Store, PathBuf) {
    let db = repo.join(".groundgraph/graph.db");
...
fn debug_kinds(nodes: &[groundgraph_core::Node]) -> Vec<(String, String)> {
    nodes
        .iter()
        .map(|n| {
            (
...
fn swift_indexer_skips_when_sourcekit_lsp_unavailable() {
    ...
    let opts = SwiftIndexOptions {
        ...
        lsp_command: Some("groundgraph_nonexistent_swift_lsp_xyz".into()),
    };
