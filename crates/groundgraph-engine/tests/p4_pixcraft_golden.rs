...
//!   1. `groundgraph init`       (P0)
//!   2. `groundgraph index`      (P2 — dart_analyzer sidecar)
//!   3. `groundgraph candidate list / review`   (P1 + P5)
//!   4. `groundgraph logic`                     (P3)
//!
...
use groundgraph_engine::business_candidates::{
    apply_review, candidate_artifact_id, list_for_review, load_business_candidates, ReviewStatus,
    ReviewVerdict,
};
use groundgraph_engine::graph::{
    build_graph_view, GraphLayer, GraphOptions, GraphStatus, GraphView,
};
use groundgraph_engine::impact::{compute_impact_with_policy, merge_confirmed_candidates};
use groundgraph_engine::init::{init_repository, InitOptions};
use groundgraph_engine::logic_confidence::{
...
fn setup_indexed_repo() -> Option<tempfile::TempDir> {
    common::setup_indexed_dart_repo("p4_pixcraft_golden", "pixcraft_iap", &["lib", "test"])
}
...
fn p4_logic_confidence_report_reflects_review_outcomes() {
    ...
    let store = groundgraph_store::Store::open(tmp.path().join(".groundgraph/graph.db")).unwrap();
    ...
    let by_id: std::collections::HashMap<String, &groundgraph_engine::LogicConfidenceItem> =
...
fn p4_impact_surfaces_accepted_candidate_when_its_evidence_changes() {
    ...
    let changed = vec![groundgraph_engine::git_diff::ChangedFile {
        path: "lib/features/paywall/paywall_screen.dart".into(),
        status: groundgraph_engine::git_diff::ChangeStatus::Modified,
        hunks: vec![groundgraph_engine::git_diff::Hunk {
            new_start: 1,
            new_end: 999,
        }],
    }];
    ...
    let mut report = compute_impact_with_policy(
        &store,
        &changed,
        groundgraph_engine::impact::ImpactPolicy::default(),
    )
    .unwrap();
