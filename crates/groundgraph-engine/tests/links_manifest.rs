...
//!
//! GroundGraph must be non-invasive: business Markdown, Dart implementation, and
//! Dart tests are indexed as facts only. Requirement relationships are declared
//! in `.groundgraph/links.yaml`.

use groundgraph_core::{artifact_id::requirement_id, EdgeKind, NodeKind};
use groundgraph_engine::checks::{compute_checks, CheckSeverity};
...
fn index_repository_uses_external_links_without_inline_annotations() {
    ...
    let store = Store::open(tmp.path().join(".groundgraph/graph.db")).unwrap();
...
fn links_manifest_can_create_requirement_without_frontmatter_doc() {
    ...
    write(
        ...
        "# Plain notes\n\nNo GroundGraph frontmatter is required here.\n",
    );
    write(
        &tmp.path().join(".groundgraph/links.yaml"),
    ...
    assert_eq!(req.source_file.as_deref(), Some(".groundgraph/links.yaml"));
}
