...
use groundgraph_engine::context_pack::{node_kind_counts, ContextOptions};
...
use groundgraph_engine::impact::ImpactReport;
use groundgraph_engine::links_indexer::{index_links, LinksIndexOptions};
use groundgraph_engine::{build_context, EngineConfig};
...
fn context_pack_includes_slice_snippets_and_edges() {
    ...
    // Index into the workspace graph database used by build_context.
    let db_path = tmp.path().join(".groundgraph/graph.db");
