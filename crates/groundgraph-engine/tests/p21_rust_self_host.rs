...
//!
//! The LSP-first adapters could never index GroundGraph's own Rust sources
...
use groundgraph_engine::{index_rust, RustIndexOptions, RUST_INDEXER_NAME};
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = <root>/crates/groundgraph-engine
...
#[test]
fn groundgraph_indexes_its_own_rust_workspace() {
    let root = workspace_root();
    assert!(
        root.join("crates").join("groundgraph-engine").exists(),
        "expected to run from the GroundGraph workspace, got {}",
        root.display()
    );
