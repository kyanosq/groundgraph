...
//!    the GroundGraph repo itself (Rust) and external Dart apps when present.
...
use groundgraph_engine::treesitter::{self, spec_for_language, TsIndexOptions};
...
fn p23_corpus_self_repo_rust_is_total_and_deterministic() {
    ...
    assert!(
        !first.is_empty(),
        "indexing the GroundGraph Rust sources should produce a graph"
    );
    assert_eq!(
        first, second,
