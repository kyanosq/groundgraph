...
fn find_node<'a>(view: &'a GraphViewModel, id: &str) -> &'a groundgraph_engine::graph::GraphNode {
    view.nodes
        .iter()
        .find(|n| n.id == id)
...
fn overview_view_emits_module_nodes_with_child_counts() {
    ...
    let modules: Vec<&groundgraph_engine::graph::GraphNode> =
...
fn overview_default_visible_marks_only_top_level_modules() {
    ...
    .unwrap();
    let visible: Vec<&groundgraph_engine::graph::GraphNode> =
