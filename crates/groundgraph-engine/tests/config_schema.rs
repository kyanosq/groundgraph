//! Schema tests for `.groundgraph.yaml`.
//!
...
use groundgraph_engine::config::{
    config_schema_notice, EngineConfig, CONFIG_SCHEMA_VERSION, DEFAULT_CONFIG_FILE_NAME,
};
use groundgraph_engine::impact::{run_impact, ImpactOptions};
...
"#;
    ...
    assert_eq!(cfg.links.path, ".groundgraph/links.yaml");
...
fn index_repository_honours_configured_docs_paths() {
    ...
    // from `.groundgraph.yaml` instead of hard-coding `docs/specs/adr`.
    ...
    let sections = store
        .list_nodes_by_kind(groundgraph_core::NodeKind::DocSection)
        .unwrap();
...
fn index_repository_honours_configured_code_paths_and_exclude() {
    ...
    let store = Store::open(root.join(".groundgraph/graph.db")).unwrap();
    let classes = store
        .list_nodes_by_kind(groundgraph_core::NodeKind::DartClass)
        .unwrap();
...
fn run_checks_honours_configured_finding_levels() {
    ...
    let mut req = groundgraph_core::Node::new(
        groundgraph_core::artifact_id::requirement_id("REQ-CHECK-1"),
        groundgraph_core::NodeKind::Requirement,
    );
    ...
    assert_eq!(
        orphan.severity,
        groundgraph_engine::checks::CheckSeverity::Info
    );
}
...
/// #72: `init` stamps every freshly-written `.groundgraph.yaml` with the current
/// schema version and emits unified `languages:` selections, so future builds
/// can detect version skew. Mirrors how the DB and every other contract is
/// versioned.
#[test]
...
#[test]
fn init_writes_groundgraph_namespace_by_default() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
    ...
    assert_eq!(outcome.config_path, root.join(".groundgraph.yaml"));
    assert_eq!(outcome.links_path, root.join(".groundgraph/links.yaml"));
    assert_eq!(outcome.graph_db_path, root.join(".groundgraph/graph.db"));
    assert_eq!(
        outcome.requirements_dir,
        root.join(".groundgraph/requirements")
    );
    assert!(root.join(".groundgraph.yaml").is_file());
    assert!(root.join(".groundgraph/graph.db").is_file());
    assert!(!root.join(".specslice.yaml").exists());
}

#[test]
fn specslice_config_is_not_loaded_as_a_workspace() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
    std::fs::create_dir_all(root.join("docs")).unwrap();
    std::fs::write(root.join("docs/req.md"), "# REQ-OLD Legacy\n").unwrap();
    std::fs::create_dir_all(root.join(".specslice")).unwrap();
    std::fs::write(
    ...
    let err = index_repository(IndexOptions::docs_only(root.to_path_buf()))
        .unwrap_err()
        .to_string();

    assert!(err.contains("groundgraph init"), "err = {err}");
    assert!(!root.join(".groundgraph/graph.db").exists());
}
...
#[test]
fn init_on_dart_repo_uses_language_selection() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
    std::fs::create_dir_all(root.join("lib")).unwrap();
...
fn index_repository_runs_swift_adapter_when_enabled_and_skips_when_lsp_missing() {
    ...
    // bogus command in `.groundgraph.yaml` is *ignored* and the adapter falls
    ...
    // override `GROUNDGRAPH_SWIFT_LSP_BIN` is always honoured (it bypasses the
    ...
    SET_MISSING_SWIFT_LSP.call_once(|| {
        std::env::set_var(
            groundgraph_engine::SWIFT_LSP_COMMAND_ENV,
            "groundgraph_missing_sourcekit_lsp",
        );
    });
...
fn p23_unified_languages_normalize_onto_legacy_switches() {
    use groundgraph_engine::config::LanguageSelection;
    let cfg = EngineConfig {
        languages: vec![
            LanguageSelection {
                id: "dart".into(),
...
fn javascript_alias_routes_through_typescript_adapter() {
    use groundgraph_engine::config::{canonical_language_id, EngineConfig, LanguageSelection};
...
fn p23_enrichment_lsp_false_routes_lsp_languages_to_structure_only() {
    use groundgraph_engine::config::{EnrichmentConfig, LanguageSelection};
    let cfg = EngineConfig {
...
fn p23_unified_languages_excludes_unlisted_dart() {
    ...
    let rust_fns = store
        .list_nodes_by_kind(groundgraph_core::NodeKind::RustFunction)
        .unwrap();
    assert!(
        rust_fns
            .iter()
