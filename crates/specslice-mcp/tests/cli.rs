//! End-to-end CLI behaviour for the `specslice-mcp` binary.
//!
//! Before this, the server ignored every flag except `--repo-root` and
//! immediately entered the stdin-blocking JSON-RPC loop — so `specslice-mcp
...
fn help_flag_prints_usage_and_exits_zero() {
    Command::cargo_bin("specslice-mcp")
        .unwrap()
        .arg("--help")
        .write_stdin("")
        .assert()
