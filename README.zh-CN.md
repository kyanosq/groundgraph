...
从 [GitHub Releases](https://github.com/kyanosq/groundgraph/releases) 下载，无需 Rust 工具链。每个包都同时包含 `groundgraph` CLI 与 `groundgraph-mcp` 服务。

**macOS / Linux** —— 解压到任意目录，将其 `bin/` 加入 `PATH`：

```bash
curl -L https://github.com/kyanosq/groundgraph/releases/latest/download/groundgraph-<版本号>-macos-universal.tar.gz | tar -xz
```

**Windows**（PowerShell）：

```powershell
Invoke-WebRequest https://github.com/kyanosq/groundgraph/releases/latest/download/groundgraph-<版本号>-windows-x86_64.zip -OutFile groundgraph.zip
Expand-Archive groundgraph.zip
```

把 `<版本号>` 换成实际版本（例如 `0.2.0`）；Linux 选择 `…-linux-x86_64.tar.gz` 或 `…-linux-aarch64.tar.gz`。

### 从源码构建

GroundGraph 是一个 Rust workspace（`rust-toolchain.toml` 已钉死工具链版本）：
